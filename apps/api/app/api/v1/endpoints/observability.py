...
platform_role_checker = RoleChecker(["Super Admin", "SRE", "Platform Admin", "DevOps"])
read_role_checker = RoleChecker(["Super Admin", "SRE", "Platform Admin", "DevOps", "Developer", "Admin"])
...
# METRICS ENDPOINTS
# =============================================================================
@router.post("/metrics", response_model=APIResponse[SystemMetricResponse], status_code=status.HTTP_201_CREATED)
...
async def query_system_metrics(
    metric_name: Optional[str] = Query(None, description="Filter metrics by name"),
    ...
    service = ObservabilityService(db)
    metrics = await service.get_system_metrics(current_user.organization_id, metric_name, duration_minutes)
    return APIResponse(
        success=True,
...
# LOGS ENDPOINTS
# =============================================================================
@router.post("/logs", response_model=APIResponse[ApplicationLogResponse], status_code=status.HTTP_201_CREATED)
...
# TRACING ENDPOINTS
# =============================================================================
@router.post("/traces", response_model=APIResponse[TraceSpanResponse], status_code=status.HTTP_201_CREATED)
...
# ALERTS ENDPOINTS
# =============================================================================
@router.post("/alerts", response_model=APIResponse[AlertResponse], status_code=status.HTTP_201_CREATED)
...
async def list_incident_alerts(
    ...
    service = ObservabilityService(db)
    alerts = await service.get_alerts(current_user.organization_id, alert_status, severity)
    return APIResponse(
        success=True,
...
# =============================================================================
@router.get("/dashboards/{dashboard_type}", response_model=APIResponse[DashboardConfigResponse])
async def retrieve_dashboard_config(
    ...
    service = ObservabilityService(db)
    config = await service.get_dashboard_config(current_user.organization_id, dashboard_type)
    if not config:
...
@router.post("/dashboards/{dashboard_type}", response_model=APIResponse[DashboardConfigResponse])
