...
from app.schemas.org_mgmt import BulkDeleteRequest, TeamCreate, TeamResponse, TeamUpdate
...
async def list_teams(
    ...
    stmt = select(Team).where(
        Team.organization_id == current_user.organization_id, Team.is_deleted == False
    )
    if department_id:
    ...
    if search:
        stmt = stmt.where(
            or_(Team.name.ilike(f"%{search}%"), Team.slug.ilike(f"%{search}%"))
        )
