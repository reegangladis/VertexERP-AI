...
async def get_collection(
    ...
    collection = await service.get_collection(
        collection_id, current_user.organization_id
    )
    if not collection:
...
async def upload_document(
    ...
    tags: str | None = Form(None),  # comma-separated list of tags
...
async def list_chat_sessions(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    service = RAGService(db)
    return await service.list_chat_sessions(
...
async def get_chat_session(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    service = RAGService(db)
    session = await service.get_chat_session(
...
@router.get(
    "/chat/sessions/{session_id}/messages", response_model=list[RAGChatMessageResponse]
)
...
async def toggle_pin_session(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    service = RAGService(db)
    success = await service.toggle_pin_session(
        session_id, current_user.organization_id, current_user.id
    )
    if not success:
        raise HTTPException(status_code=404, detail="Chat session not found")
    return await service.get_chat_session(
        session_id, current_user.organization_id, current_user.id
    )
