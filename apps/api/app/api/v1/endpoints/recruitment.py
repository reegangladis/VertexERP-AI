...
async def create_job(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    repo = RecruitmentJobRepository(db)
    job = await repo.create({"organization_id": current_user.organization_id, **payload.dict()})
    return standard_json_response(
...
async def create_candidate(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    repo = CandidateRepository(db)
    candidate = await repo.create({"organization_id": current_user.organization_id, **payload.dict()})
    return standard_json_response(
        status_code=status.HTTP_201_CREATED,
        success=True,
...
async def create_application(
    ...
    repo = ApplicationRepository(db)
    app_obj = await repo.create({**payload.dict(), "date_applied": datetime.now().date()})
    return standard_json_response(
        status_code=status.HTTP_201_CREATED,
        success=True,
