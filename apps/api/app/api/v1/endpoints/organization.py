...
async def get_my_org(
    current_user: User = Depends(get_current_user), org_service=Depends(get_org_service)
):
...
async def get_tenant_settings(
    ...
    if not settings_obj:
        settings_obj = await tenant_service.repository.create(
            {"organization_id": org.id, "currency": "USD", "locale": "en_US"}
        )
    return standard_json_response(
        status_code=status.HTTP_200_OK,
        success=True,
...
@router.get(
    "/me/security-settings", response_model=APIResponse[SecuritySettingResponse]
)
...
async def get_org_settings(
    ...
    if not settings:
        settings = await service.configure_settings(
            current_user.organization_id,
            {"timezone": "UTC", "locale": "en_US", "currency": "USD"},
        )
...
async def update_org_settings(
    ...
    service = OrgSettingService(OrgSettingRepository(db))
    payload_data = (
        payload if isinstance(payload, dict) else payload.model_dump(exclude_unset=True)
    )
    settings = await service.configure_settings(
        current_user.organization_id, payload_data
    )
...
async def seed_enterprise_data(
    ...
    if not current_user.organization_id:
        raise HTTPException(
            status_code=400, detail="User is not bound to an organization"
        )
    ...
    remote_hub = Location(
        ...
        country="USA",
        city="San Francisco",
        address_line1="10 Market St",
    )
    db.add_all([hq, warehouse, remote_hub])

    # 1b. Seed Business Units
    bu_enterprise = BusinessUnit(
        organization_id=org_id,
        name="Enterprise Solutions",
        slug="enterprise-solutions",
        code="BU-ENT",
        description="Enterprise AI & ERP software division",
    )
    bu_consumer = BusinessUnit(
        organization_id=org_id,
        name="Consumer Cloud",
        slug="consumer-cloud",
        code="BU-CNS",
        description="Consumer cloud software division",
    )
    ...
    ui_team = Team(
        ...
        slug="ui-ux",
        description="Frontend interfaces",
    )
    db.add_all([core_team, ui_team])

    # 4. Seed Designations
    ceo_desig = Designation(
        organization_id=org_id,
    ...
    calendar = BusinessCalendar(
        organization_id=org_id, name="Fiscal Calendar 2026", year=2026, is_active=True
    )
    db.add(calendar)
    await db.flush()
    ...
    vp_user = User(
        organization_id=org_id,
        designation_id=vp_desig.id,
        manager_id=ceo_user.id,
        first_name="Emma",
        last_name="Stone",
        username="emma.vp",
        email="vp@vertex.ai",
    ...
    dir_user = User(
        organization_id=org_id,
        designation_id=dir_desig.id,
        manager_id=vp_user.id,
        first_name="Bruce",
        last_name="Wayne",
        username="bruce.dir",
        email="director@vertex.ai",
        password_hash=pwd,
        email_verified=True,
    )
    db.add(dir_user)
    ...
    mgr_user = User(
        organization_id=org_id,
        designation_id=mgr_desig.id,
        manager_id=dir_user.id,
        first_name="Clark",
        last_name="Kent",
        username="clark.mgr",
        email="manager@vertex.ai",
        password_hash=pwd,
        email_verified=True,
    )
    db.add(mgr_user)
    ...
    lead_user = User(
        organization_id=org_id,
        designation_id=lead_desig.id,
        manager_id=mgr_user.id,
        first_name="Peter",
        last_name="Parker",
        username="peter.lead",
        email="lead@vertex.ai",
        password_hash=pwd,
        email_verified=True,
    )
    db.add(lead_user)
    ...
    emp_user = User(
        organization_id=org_id,
        designation_id=emp_desig.id,
        manager_id=lead_user.id,
        first_name="Tony",
        last_name="Stark",
        username="tony.emp",
        email="employee@vertex.ai",
        password_hash=pwd,
        email_verified=True,
    )
    db.add(emp_user)
    ...
    dir_emp = Employee(
        organization_id=org_id,
        user_id=dir_user.id,
        employee_code="EMP-DIR-03",
        employment_type="full-time",
        status="active",
        date_joined=date(2025, 3, 1),
        branch_id=head_branch.id,
        department_id=eng_dept.id,
        designation_id=dir_desig.id,
        manager_id=vp_emp.id,
    )
    mgr_emp = Employee(
        organization_id=org_id,
        user_id=mgr_user.id,
        employee_code="EMP-MGR-04",
        employment_type="full-time",
        status="active",
        ...
        department_id=eng_dept.id,
        designation_id=mgr_desig.id,
        manager_id=dir_emp.id,
    )
    lead_emp = Employee(
        organization_id=org_id,
        user_id=lead_user.id,
        employee_code="EMP-TL-05",
        employment_type="full-time",
        status="active",
        date_joined=date(2025, 5, 1),
        branch_id=head_branch.id,
        department_id=eng_dept.id,
        designation_id=lead_desig.id,
    ...
    for emp_obj, email, phone in [
        ...
        prof = EmployeeProfile(
            ...
            gender=(
                "Male"
                if emp_obj in [ceo_emp, emp_emp, dir_emp, mgr_emp, lead_emp]
                else "Female"
            ),
            nationality="USA",
    ...
    al = LeaveType(
        organization_id=org_id, name="Annual Leave", code="AL", days_per_year=20.0
    )
    sl = LeaveType(
        organization_id=org_id, name="Sick Leave", code="SL", days_per_year=10.0
    )
    db.add_all([al, sl])
    await db.flush()
    ...
    cand = Candidate(
        ...
        phone="555-1234",
        headline="React expert with 5 years experience",
        skills={"list": ["React", "TypeScript", "TailwindCSS"]},
    )
    db.add(cand)
    await db.flush()
    ...
    db.add(
        Goal(
            employee_id=emp_emp.id,
            title="Complete ERP Dashboard Integration",
            description="Implement React tables and charts telemetry",
    ...
    review = PerformanceReview(
        employee_id=emp_emp.id,
        reviewer_id=lead_emp.id,
        review_cycle="2026_Q2",
        rating=4.50,
        manager_feedback="Great work delivering on clean architecture targets.",
        self_assessment="Feel like I'm pacing well.",
    )
    db.add(review)
    ...
    course = TrainingCourse(
        ...
        description="Learn secure code review rules",
    ...
    db.add(
        Attendance(
            employee_id=emp_emp.id,
            date=date(2026, 7, 24),
            check_in=datetime(2026, 7, 24, 9, 0),
            check_out=datetime(2026, 7, 24, 17, 30),
            total_hours=8.5,
            status="present",
    ...
    from app.models.crm_customer import (
        Contact,
        Customer,
    )
    from app.models.crm_deal import Deal, Opportunity, Quotation
    ...
    l1 = Lead(
        ...
        phone="42-42-42",
        company="Magrathea Inc",
        status="new",
        lead_source_id=web_src.id,
        score=80,
    )
    l2 = Lead(
        organization_id=org_id,
    ...
    db.add(
        LeadActivity(
            ...
            description="Lead generated online.",
        )
    )
    db.add(
        LeadActivity(
            ...
            type="call",
            title="Initial Call",
            description="Discussed guide subscriptions.",
        )
    )
    ...
    alfred = Contact(
        organization_id=org_id,
        customer_id=wayne_cust.id,
        first_name="Alfred",
        last_name="Pennyworth",
        email="alfred@wayne.com",
        phone="555-0200",
        job_title="Chief Butler",
    ...
    opp1 = Opportunity(
        ...
        description="Clean energy supply contract",
        stage="proposal",
        close_date=date(2026, 12, 31),
    )
    opp2 = Opportunity(
        organization_id=org_id,
        title="Batmobile Tech License",
        description="Advanced automotive features",
        stage="qualification",
    ...
    db.add(
        CRMTask(
            organization_id=org_id,
            customer_id=stark_cust.id,
            title="Follow up with Pepper on terms",
    ...
    db.add(
        Meeting(
            organization_id=org_id,
            customer_id=wayne_cust.id,
            title="Quarterly Alignment Review",
            scheduled_at=datetime(2026, 8, 20, 10, 0),
            duration_minutes=60,
            location_or_url="Zoom Link",
        )
    )
    ...
    from app.models.inventory_product import Brand, Product, ProductCategory, Unit
    ...
    cat_mech = ProductCategory(
        organization_id=org_id, name="Mechanical Parts", code="MECH"
    )
    ...
    p1 = Product(
        ...
        brand_id=brand_stk.id,
        unit_id=unit_pcs.id,
        name="Arc Reactor Core V3",
        sku="STK-ARC-V3",
        barcode="1234567890",
        status="active",
        safety_stock=5,
    ...
    p2 = Product(
        ...
        brand_id=brand_wyn.id,
        unit_id=unit_pcs.id,
        name="Batmobile Suspension System",
        sku="WYN-SUSP-B1",
        barcode="9876543210",
        status="active",
        safety_stock=2,
    ...
    w1 = Warehouse(
        organization_id=org_id,
        name="Stark Central Vault",
        code="STK-CTR-VLT",
        address="Malibu, CA",
        capacity_cubic_meters=5000.0,
    )
    w2 = Warehouse(
        organization_id=org_id,
        name="Wayne Gotham Silo",
        code="WYN-GOTH-SLO",
        address="Gotham City",
        capacity_cubic_meters=12000.0,
    )
    db.add_all([w1, w2])
    await db.flush()

    bin1 = WarehouseBin(
        warehouse_id=w1.id, zone="A", rack="01", shelf="03", bin_code="A-01-03"
    )
    bin2 = WarehouseBin(
        warehouse_id=w2.id, zone="B", rack="04", shelf="02", bin_code="B-04-02"
    )
    ...
    po_item = PurchaseOrderItem(
        purchase_order_id=po.id, product_id=p1.id, quantity=10, unit_price=2500.0
    )
    db.add(po_item)
    ...
    db.add(
        InventoryAdjustment(
            organization_id=org_id,
            warehouse_id=w1.id,
            adjusted_by_id=ceo_user.id,
            status="pending",
        )
    )
    db.add(
        InventoryCount(organization_id=org_id, warehouse_id=w2.id, status="in_progress")
    )
