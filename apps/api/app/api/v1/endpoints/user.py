...
async def list_users(
    ...
    users, total = await user_service.get_multi(
        skip=skip, limit=limit, filters=filters, sort=sort_list
    )
...
async def get_user_sessions(
    ...
    if user_id != current_user.id and not any(
        r.name == "Super Admin" for r in current_user.roles
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You do not have access to view this user's sessions",
        )
...
async def terminate_all_sessions(
    ...
    if user_id != current_user.id and not any(
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You do not have access to terminate this user's sessions",
        )
