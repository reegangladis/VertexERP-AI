...
async def create_workflow(
    ...
    org_id = _get_org_id()
    graph = payload.graph_definition.model_dump() if payload.graph_definition else {"nodes": [], "edges": [], "layout": {}}
    workflow = await repo.create_workflow(org_id, {
        "name": payload.name,
...
async def list_workflows(
    ...
    repo = WorkflowRepository(db)
    return await repo.list_workflows(_get_org_id(), status=status, category=category, skip=skip, limit=limit)
...
async def update_workflow(
    ...
    if not workflow:
        raise HTTPException(status_code=404, detail="Workflow not found")
    updated = await repo.update_workflow(workflow, payload.model_dump(exclude_unset=True))
    await db.commit()
    return updated
...
async def create_version(
    ...
    repo = WorkflowRepository(db)
    org_id = _get_org_id()
    version = await repo.create_version(org_id, {
...
async def execute_workflow(
    ...
    if workflow.status != "published":
        raise HTTPException(status_code=400, detail="Only published workflows can be executed")
...
async def import_workflow(
    ...
    for v in payload.get("versions", []):
        await repo.create_version(org_id, {
            "workflow_id": workflow.id,
            "version_number": v.get("version_number", "1.0.0"),
            "graph_definition": v.get("graph_definition", {"nodes": [], "edges": [], "layout": {}}),
            "is_published": False,
