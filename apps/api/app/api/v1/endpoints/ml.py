...
from app.models.ml import MLFeatureMetadata
...
# =============================================================================
@router.post(
    "/models", response_model=MLModelResponse, status_code=status.HTTP_201_CREATED
)
...
@router.post(
    "/training-jobs",
    response_model=MLTrainingJobResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.post(
    "/experiments/{experiment_id}/runs", response_model=MLExperimentRunResponse
)
...
async def predict_business_module(
    ...
    if key in ["attrition", "employee_attrition"]:
        ...
        raise HTTPException(
            status_code=400, detail=f"Unsupported business module key '{key}'"
        )
...
@router.post(
    "/feature-metadata",
    response_model=MLFeatureMetadataResponse,
    status_code=status.HTTP_201_CREATED,
)
