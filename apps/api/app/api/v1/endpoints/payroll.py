...
@router.get(
    "/salary-structures", response_model=APIResponse[list[SalaryStructureResponse]]
)
...
async def create_salary_structure(
    payload: SalaryStructureCreate, db=Depends(get_db_session)
):
...
async def get_payroll_summary(
    ...
    return standard_json_response(
        ...
        data={
            ...
            "latest_run": {
                "period": (
                    f"{latest_run.period_month}/{latest_run.period_year}"
                    if latest_run
                    else "N/A"
                ),
                ...
                "total_deductions": (
                    float(latest_run.total_deductions) if latest_run else 0.0
                ),
