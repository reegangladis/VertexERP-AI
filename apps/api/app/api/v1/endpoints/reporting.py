...
from app.schemas.org_mgmt import ReportingTreeNode, EmployeeNode
...
async def get_reporting_tree(
    ...
    stmt = (
        select(User)
        .where(User.organization_id == current_user.organization_id, User.is_deleted == False)
...
async def assign_manager(
    ...
    if manager_id:
        ...
        stmt = select(User).where(User.id == manager_id, User.organization_id == current_user.organization_id)
        res = await db.execute(stmt)
        mgr_obj = res.scalar_one_or_none()
        if not mgr_obj:
            raise HTTPException(status_code=404, detail="Manager not found in organization")
    ...
    return standard_json_response(
        ...
        data={"user_id": str(user_id), "manager_id": str(manager_id) if manager_id else None}
    )
