...
@router.post(
    "/restore", response_model=RestoreJobOut, status_code=status.HTTP_201_CREATED
)
async def trigger_restore_job(
    ...
    res = backup_service.simulate_disaster_recovery_restore(
        str(payload.backup_job_id), payload.target_environment
    )
