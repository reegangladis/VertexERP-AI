...
# =============================================================================
@router.post("/deployments", response_model=MLDeploymentResponse, status_code=status.HTTP_201_CREATED)
async def create_deployment(
    ...
    return await service.create_deployment(current_user.organization_id, payload, current_user.email)
...
async def update_traffic(
    ...
    service = MLOpsService(db)
    updated = await service.update_traffic(deployment_id, payload.target_traffic_percentage, current_user.email)
    if not updated:
        raise HTTPException(status_code=404, detail="Deployment not found")
    return updated


@router.post("/deployments/{deployment_id}/rollback", response_model=MLDeploymentResponse)
...
# PIPELINE ENDPOINTS
# =============================================================================
@router.post("/pipelines/templates", response_model=PipelineTemplateResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/pipelines/runs", response_model=PipelineRunResponse, status_code=status.HTTP_201_CREATED)
...
# =============================================================================
@router.post("/approvals", response_model=ModelApprovalResponse, status_code=status.HTTP_201_CREATED)
...
async def list_approvals(
    status: Optional[str] = Query(None, description="Filter queue: PENDING, APPROVED, REJECTED"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
# =============================================================================
@router.post("/monitoring/{deployment_id}/metrics", response_model=ModelMonitoringMetricResponse, status_code=status.HTTP_201_CREATED)
async def ingest_telemetry_metric(
    ...
    service = MLOpsService(db)
    return await service.ingest_metric(current_user.organization_id, deployment_id, payload)


@router.get("/monitoring/{deployment_id}/metrics", response_model=List[ModelMonitoringMetricResponse])
...
@router.post("/monitoring/{deployment_id}/drift", response_model=DriftReportResponse, status_code=status.HTTP_201_CREATED)
async def evaluate_drift(
    ...
    service = MLOpsService(db)
    return await service.evaluate_drift(current_user.organization_id, deployment_id, payload)


@router.get("/monitoring/{deployment_id}/drift", response_model=List[DriftReportResponse])
...
# =============================================================================
@router.post("/retraining", response_model=RetrainingJobResponse, status_code=status.HTTP_201_CREATED)
...
async def list_retraining_history(
    model_id: Optional[uuid.UUID] = Query(None, description="Filter retraining history by Model ID"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
