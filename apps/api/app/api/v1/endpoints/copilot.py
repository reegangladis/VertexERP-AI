...
from app.core.dependencies import get_db_session, get_current_user, get_redis
...
async def create_session(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User is not bound to any organization")
...
async def send_chat_message(
    ...
    try:
        ...
        raise HTTPException(status_code=429, detail=str(run_err))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal chat processing failure: {e}")
...
async def delete_prompt_template(
    ...
    if not success:
        raise HTTPException(status_code=403, detail="Cannot delete template or template not found")
...
@router.get(
    "/analytics",
    summary="Retrieve analytics dashboard metrics for the Copilot service"
)
