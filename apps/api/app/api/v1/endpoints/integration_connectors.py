...
async def list_connectors(
    ...
    return await repo.list_connectors(
        category=category, provider=provider, status=status_filter
    )


@router.post(
    "/connectors", response_model=ConnectorOut, status_code=status.HTTP_201_CREATED
)
...
@router.post(
    "/configs", response_model=ConnectorConfigOut, status_code=status.HTTP_201_CREATED
)
