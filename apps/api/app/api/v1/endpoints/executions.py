...
async def list_executions(
    ...
    return await repo.list_executions(
        _get_org_id(), workflow_id=workflow_id, status=status, skip=skip, limit=limit
    )
...
async def list_logs(
    ...
    return await repo.list_logs(
        execution_id, log_level=log_level, skip=skip, limit=limit
    )
...
async def retry_execution(
    ...
    if execution.status not in ("failed", "cancelled"):
        raise HTTPException(
            status_code=400, detail="Only failed or cancelled executions can be retried"
        )
    engine = WorkflowEngine(db)
