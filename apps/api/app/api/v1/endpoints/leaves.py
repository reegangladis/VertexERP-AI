...
from app.repositories.hr_mgmt import LeaveRequestRepository, LeaveBalanceRepository, LeaveTypeRepository
...
async def get_leave_service(db=Depends(get_db_session)):
    return LeaveService(LeaveRequestRepository(db), LeaveBalanceRepository(db), LeaveTypeRepository(db))
...
async def create_leave_type(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=400, detail="User not bound to organization")
    ltype = await service.type_repo.create({"organization_id": current_user.organization_id, **payload.dict()})
    return standard_json_response(
...
async def approve_leave_request(
    ...
    try:
        req = await service.process_approval(id, mock_approver_id, payload.status, payload.approval_comment)
        return standard_json_response(
            status_code=status.HTTP_200_OK,
            success=True,
