...
from app.models.crm_deal import Quotation
...
async def list_quotations(deal_id: uuid.UUID | None = None, db=Depends(get_db_session)):
...
async def create_quotation(payload: QuotationCreate, db=Depends(get_db_session)):
...
async def update_quotation_status(
    id: uuid.UUID, payload: QuotationStatusUpdate, db=Depends(get_db_session)
):
    repo = QuotationRepository(db)
...
@router.post(
    "/quotations/{id}/convert-to-order", response_model=APIResponse[SalesOrderResponse]
)
async def convert_quotation_to_sales_order(
    ...
    try:
        order = await service.convert_quotation_to_order(
...
async def get_deal(
    ...
    if (
        not deal
        or deal.organization_id != current_user.organization_id
        or deal.is_deleted
    ):
...
async def process_deal_result(
    ...
    try:
        deal = await service.process_deal_result(
            id, payload.status, payload.won_lost_reason
        )
        return standard_json_response(
            status_code=status.HTTP_200_OK,
            success=True,
