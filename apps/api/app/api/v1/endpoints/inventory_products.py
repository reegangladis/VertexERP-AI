...
from app.models.inventory_product import Brand, Product, Unit
...
async def get_product_service(db=Depends(get_db_session)):
    return ProductService(
        ProductRepository(db), ProductCategoryRepository(db), UnitRepository(db)
    )
...
async def list_brands(
    ...
    stmt = select(Brand).where(
        Brand.organization_id == current_user.organization_id, Brand.is_deleted == False
    )
...
async def list_units(
    ...
    stmt = select(Unit).where(
        Unit.organization_id == current_user.organization_id, Unit.is_deleted == False
    )
    res = await db.execute(stmt)
...
async def export_products(
    ...
    writer.writerow(
        ["name", "sku", "barcode", "status", "safety_stock", "reorder_level"]
    )
    for p in products:
        writer.writerow(
            [p.name, p.sku, p.barcode or "", p.status, p.safety_stock, p.reorder_level]
        )
...
async def bulk_upload_products(
    ...
    try:
        ...
        count = await service.bulk_import_products_csv(
...
async def get_inventory_summary(
    ...
    low_stock = sum(
        ...
        for p in products
        if any(
            s.product_id == p.id and s.on_hand <= p.reorder_level for s in stock_levels
        )
    )
    out_of_stock = sum(
        1
        for p in products
        if not any(s.product_id == p.id and s.on_hand > 0 for s in stock_levels)
    )
    ...
    return standard_json_response(
        ...
        data={
            ...
            "pending_purchase_orders": len(
                [po for po in pos if po.status in ["draft", "ordered"]]
            ),
        },
    )
...
async def get_product(
    ...
    if (
        not product
        or product.organization_id != current_user.organization_id
