...
class RefreshTokenRepository(BaseRepository[RefreshToken]):
    ...
    async def get_by_token(self, token: str) -> RefreshToken | None:
        stmt = select(RefreshToken).where(RefreshToken.token == token, RefreshToken.is_deleted == False)
        result = await self.db.execute(stmt)
...
class TrustedDeviceRepository(BaseRepository[TrustedDevice]):
    ...
    async def get_by_fingerprint(self, user_id: uuid.UUID, fingerprint: str) -> TrustedDevice | None:
