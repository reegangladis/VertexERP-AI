...
class MLRepository:
    ...
    async def get_model_by_code(self, organization_id: uuid.UUID, model_code: str) -> Optional[MLModel]:
        stmt = (
            select(MLModel)
            .options(selectinload(MLModel.versions))
            .where(MLModel.organization_id == organization_id, MLModel.model_code == model_code)
        )
        res = await self.db.execute(stmt)
        return res.scalar_one_or_none()
    ...
    async def get_model_version_by_id(self, version_id: uuid.UUID) -> Optional[ModelVersion]:
        stmt = select(ModelVersion).where(ModelVersion.id == version_id)
        res = await self.db.execute(stmt)
        return res.scalar_one_or_none()

    async def update_version_status(self, version_id: uuid.UUID, status: str, approval_status: str, approved_by: Optional[str] = None) -> Optional[ModelVersion]:
        stmt = (
            update(ModelVersion)
    ...
    async def get_training_jobs(self, organization_id: uuid.UUID) -> List[MLTrainingJob]:
        stmt = (
    ...
    async def get_training_job_by_id(self, job_id: uuid.UUID) -> Optional[MLTrainingJob]:
        stmt = (
            select(MLTrainingJob)
            .options(selectinload(MLTrainingJob.runs))
    ...
    async def get_predictions(self, organization_id: uuid.UUID, limit: int = 100) -> List[MLPrediction]:
        stmt = (
            select(MLPrediction)
    ...
    async def create_prediction_history(self, history: MLPredictionHistory) -> MLPredictionHistory:
        self.db.add(history)
        await self.db.commit()
    ...
    async def get_experiment_by_id(self, experiment_id: uuid.UUID) -> Optional[MLExperiment]:
        stmt = (
            select(MLExperiment)
            .options(selectinload(MLExperiment.runs))
    ...
    # =========================================================================
    async def create_evaluation_metric(self, metric: MLEvaluationMetric) -> MLEvaluationMetric:
        self.db.add(metric)
        await self.db.commit()
        await self.db.refresh(metric)
        return metric

    async def get_metrics_by_version(self, model_version_id: uuid.UUID) -> List[MLEvaluationMetric]:
        stmt = (
    ...
    async def create_feature_metadata(self, metadata: MLFeatureMetadata) -> MLFeatureMetadata:
        self.db.add(metadata)
        await self.db.commit()
        await self.db.refresh(metadata)
        return metadata

    async def get_feature_metadata_list(self, organization_id: uuid.UUID) -> List[MLFeatureMetadata]:
        stmt = (
