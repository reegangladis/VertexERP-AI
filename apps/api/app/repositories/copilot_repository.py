...
class CopilotRepository:
    ...
    async def get_active_prompt(
        self, org_id: uuid.UUID, prompt_type: str, department: Optional[str] = None
        ...
        query = (
            ...
            .order_by(CopilotPrompt.organization_id.desc(), CopilotPrompt.version.desc())
        )
        result = await self.db.execute(query)
    ...
    # --- Tool Registry CRUD ---
    async def get_tool(self, tool_id: uuid.UUID) -> Optional[ToolRegistry]:
    ...
    async def list_tool_executions(self, session_id: uuid.UUID) -> Sequence[ToolExecution]:
        query = (
    ...
    # --- Feedback CRUD ---
    async def create_feedback(self, feedback: ConversationFeedback) -> ConversationFeedback:
        self.db.add(feedback)
        await self.db.flush()
        await self.db.refresh(feedback)
        return feedback

    async def get_feedback_by_message(self, message_id: uuid.UUID) -> Optional[ConversationFeedback]:
        query = select(ConversationFeedback).where(ConversationFeedback.message_id == message_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    # --- Metadata CRUD ---
    async def create_metadata(self, metadata: ConversationMetadata) -> ConversationMetadata:
        self.db.add(metadata)
        await self.db.flush()
        await self.db.refresh(metadata)
        return metadata

    async def list_metadata(self, session_id: uuid.UUID) -> Sequence[ConversationMetadata]:
        query = select(ConversationMetadata).where(ConversationMetadata.session_id == session_id)
        result = await self.db.execute(query)
        return result.scalars().all()
