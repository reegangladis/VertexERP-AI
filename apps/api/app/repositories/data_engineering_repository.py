...
class DataEngineeringRepository:
    ...
    async def get_jobs_by_org(self, organization_id: uuid.UUID) -> List[ETLJob]:
        stmt = select(ETLJob).where(ETLJob.organization_id == organization_id).order_by(ETLJob.name)
        res = await self.db.execute(stmt)
        return list(res.scalars().all())

    async def get_job_by_id(self, job_id: uuid.UUID) -> Optional[ETLJob]:
        stmt = select(ETLJob).options(selectinload(ETLJob.runs)).where(ETLJob.id == job_id)
        res = await self.db.execute(stmt)
    ...
    async def update_run(self, run_id: uuid.UUID, updates: Dict[str, Any]) -> Optional[ETLRun]:
        stmt = update(ETLRun).where(ETLRun.id == run_id).values(**updates).execution_options(synchronize_session="fetch")
    ...
    async def get_runs_by_job(self, job_id: uuid.UUID, limit: int = 50) -> List[ETLRun]:
        stmt = select(ETLRun).where(ETLRun.job_id == job_id).order_by(desc(ETLRun.start_time)).limit(limit)
        res = await self.db.execute(stmt)
        return list(res.scalars().all())
    ...
    async def get_run_logs(self, run_id: uuid.UUID) -> List[PipelineLog]:
        stmt = select(PipelineLog).where(PipelineLog.run_id == run_id).order_by(PipelineLog.timestamp)
        res = await self.db.execute(stmt)
        return list(res.scalars().all())
    ...
    async def get_datasets_by_org(self, organization_id: uuid.UUID) -> List[Dataset]:
        stmt = select(Dataset).where(Dataset.organization_id == organization_id).order_by(Dataset.name)
        res = await self.db.execute(stmt)
        return list(res.scalars().all())
    ...
    async def get_dataset_by_id(self, dataset_id: uuid.UUID) -> Optional[Dataset]:
        stmt = (
            select(Dataset)
            .options(selectinload(Dataset.versions), selectinload(Dataset.metadata_columns))
    ...
    async def add_metadata_catalog_entry(self, entry: MetadataCatalog) -> MetadataCatalog:
        self.db.add(entry)
        await self.db.commit()
    ...
    async def get_feature_groups_by_org(self, organization_id: uuid.UUID) -> List[FeatureGroup]:
        stmt = (
            select(FeatureGroup)
    ...
    # DATA QUALITY
    # =========================================================================
    async def create_quality_report(self, report: DataQualityReport) -> DataQualityReport:
        self.db.add(report)
        await self.db.commit()
        await self.db.refresh(report)
        return report

    async def get_quality_reports_by_org(self, organization_id: uuid.UUID, limit: int = 50) -> List[DataQualityReport]:
        stmt = select(DataQualityReport).where(DataQualityReport.organization_id == organization_id).order_by(desc(DataQualityReport.created_at)).limit(limit)
        res = await self.db.execute(stmt)
        return list(res.scalars().all())
    ...
    async def get_data_lake_objects(self, organization_id: uuid.UUID, zone: Optional[str] = None) -> List[DataLakeObject]:
        stmt = select(DataLakeObject).where(DataLakeObject.organization_id == organization_id)
        if zone:
    ...
    async def get_mdm_golden_records(self, organization_id: uuid.UUID, entity_type: Optional[str] = None) -> List[MDMGoldenRecord]:
        stmt = select(MDMGoldenRecord).where(MDMGoldenRecord.organization_id == organization_id)
        if entity_type:
