...
class AnalyticsRepository(BaseRepository[AnalyticsDashboard]):
    ...
    async def get_kpis(
        ...
        query = (
            select(KPI)
            .options(selectinload(KPI.values))
            .where(
    ...
    async def get_kpi_values(
        self, kpi_id: uuid.UUID, limit: int = 12
    ) -> list[KPIValue]:
        query = (
            select(KPIValue)
            .where(
            ...
            .order_by(KPIValue.period_start.asc())
            .limit(limit)
        )
        result = await self.session.execute(query)
        return list(result.scalars().all())
    ...
    async def get_saved_reports(
        ...
        query = (
            select(SavedReport)
            .options(selectinload(SavedReport.report))
            .where(
    ...
    async def search_analytics(
        ...
        dashboards_q = (
            ...
            .where(
                ...
                AnalyticsDashboard.deleted_at.is_(None),
                or_(
                    AnalyticsDashboard.title.ilike(pattern),
                    AnalyticsDashboard.description.ilike(pattern),
                ),
            )
            .limit(10)
        )
