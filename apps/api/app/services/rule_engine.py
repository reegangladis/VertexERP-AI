...
class RuleEngine:
    ...
    async def evaluate_rules(
        ...
        rules = await self.repo.list_rules(org_id, rule_group=request.rule_group, is_active=True)

        triggered_actions: List[Dict[str, Any]] = []
        matched_rule_ids: List[uuid.UUID] = []

        for rule in sorted(rules, key=lambda r: r.priority):
            matched = self._evaluate_conditions(rule.conditions_json, request.context_data)
            if matched:
    ...
    async def test_rule(
        ...
        return {
            ...
            "rule_name": rule.name,
            "matched": matched,
            "triggered_actions": rule.actions_json.get("actions", []) if matched else [],
        }

    # ─── Core Evaluator ──────────────────────────────────────────────────────────
    def _evaluate_conditions(self, conditions_json: Dict[str, Any], context: Dict[str, Any]) -> bool:
        """
    ...
    def validate_conditions_schema(self, conditions_json: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        for cond in conditions_json.get("conditions", []):
            if not all(k in cond for k in ["field", "operator"]):
                return False, f"Each condition must have 'field' and 'operator'. Got: {cond}"
            valid_ops = [
                "==", "!=", ">", "<", ">=", "<=",
