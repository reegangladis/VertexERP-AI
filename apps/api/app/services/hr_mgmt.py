...
class EmployeeService(BaseService[Employee, EmployeeRepository]):
    def __init__(self, repository: EmployeeRepository, profile_repo: EmployeeProfileRepository):
        super().__init__(repository)
        self.profile_repo = profile_repo

    async def create_employee(self, org_id: uuid.UUID, data: Dict[str, Any]) -> Employee:
        # Check code duplicate
        existing = await self.repository.get_code(org_id, data.get("employee_code"))
        if existing:
            raise HRServiceException(f"Employee code {data.get('employee_code')} already registered.")
    ...
    async def update_employee(self, emp_id: uuid.UUID, data: Dict[str, Any]) -> Employee:
    ...
    async def bulk_import_csv(self, org_id: uuid.UUID, file_content: bytes) -> int:
        ...
        for row in reader:
            ...
            emp_dict = {
                ...
                "date_joined": date.fromisoformat(row.get("date_joined", date.today().isoformat())),
            }
...
class AttendanceService(BaseService[Attendance, AttendanceRepository]):
    ...
    async def check_in(self, employee_id: uuid.UUID, check_in_time: datetime) -> Attendance:
    ...
    async def check_out(self, employee_id: uuid.UUID, check_out_time: datetime) -> Attendance:
...
class LeaveService(BaseService[LeaveRequest, LeaveRequestRepository]):
    def __init__(self, repository: LeaveRequestRepository, balance_repo: LeaveBalanceRepository, type_repo: LeaveTypeRepository):
        super().__init__(repository)
        self.balance_repo = balance_repo
        self.type_repo = type_repo

    async def submit_request(self, org_id: uuid.UUID, data: Dict[str, Any]) -> LeaveRequest:
        ...
        if not balance or balance.remaining < total_days:
            raise HRServiceException(f"Insufficient leave balance. Required: {total_days}, Available: {balance.remaining if balance else 0.0}")
    ...
    async def process_approval(self, request_id: uuid.UUID, approver_id: uuid.UUID, status: str, comment: Optional[str]) -> LeaveRequest:
