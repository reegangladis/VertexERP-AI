...
class WorkflowEngine:
    ...
    async def trigger_workflow(
        ...
        execution = await self.repo.create_execution(org_id, {
        ...
        await self._log(org_id, execution.id, None, "INFO", f"Workflow execution started. trigger={trigger_type}")
        ...
        try:
            ...
            execution = await self.repo.update_execution(execution, {
                "status": "completed",
                "end_time": datetime.now(UTC),
                "output_payload": output,
                "duration_ms": self._elapsed_ms(execution.start_time),
            })
            await self._log(org_id, execution.id, None, "INFO", "Workflow execution completed successfully.")
    ...
    async def cancel_execution(self, execution: WorkflowExecution, org_id: Optional[uuid.UUID]) -> WorkflowExecution:
        return await self.repo.update_execution(execution, {
            "status": "cancelled",
            "end_time": datetime.now(UTC),
        })
    ...
    def _find_entry_nodes(self, nodes: List[Dict], edges: List[Dict]) -> List[str]:
        ...
        return [n["id"] for n in nodes if n["id"] not in targets] or ([nodes[0]["id"]] if nodes else [])
    ...
    async def _execute_node(
        ...
        step = await self.repo.create_step(org_id, {
        ...
        try:
            ...
            await self.repo.update_step(step, {"status": "completed", "output_data": output, "duration_ms": duration})
            await self._log(org_id, execution.id, node_id, "INFO", f"Step '{step_name}' completed in {duration:.1f}ms.")
        except Exception as exc:
            retry_max = node_data.get("config", {}).get("max_retries", 0)
            if step.retry_count < retry_max:
                await self.repo.update_step(step, {"retry_count": step.retry_count + 1, "status": "running"})
                await asyncio.sleep(2)
                return await self._execute_node(org_id, execution, node, context)
            await self.repo.update_step(step, {
                "status": "failed", "error_details": str(exc),
                "duration_ms": self._elapsed_ms(start)
            })
            await self._log(org_id, execution.id, node_id, "ERROR", f"Step '{step_name}' failed: {exc}")
            raise
    ...
    async def _handle_action(self, config: Dict, context: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "context_snapshot": {k: v for k, v in context.items() if not k.startswith("__")},
        }
    ...
    async def _handle_approval_node(self, config: Dict, context: Dict) -> Dict[str, Any]:
    ...
    async def _handle_ai_copilot(self, config: Dict, context: Dict) -> Dict[str, Any]:
        ...
        return {"ai_copilot_executed": True, "prompt": prompt, "response": "[AI Copilot Response Placeholder]"}

    async def _handle_rag_search(self, config: Dict, context: Dict) -> Dict[str, Any]:
        query = config.get("query", "")
        return {"rag_search_executed": True, "query": query, "results": "[RAG Search Results Placeholder]"}

    async def _handle_ml_prediction(self, config: Dict, context: Dict) -> Dict[str, Any]:
        model_id = config.get("model_id", "")
        return {"ml_prediction_executed": True, "model_id": model_id, "prediction": "[ML Prediction Placeholder]"}

    async def _handle_external_api(self, config: Dict, context: Dict) -> Dict[str, Any]:
        url = config.get("url", "")
        method = config.get("method", "GET")
        return {"external_api_called": True, "url": url, "method": method, "response": "[External API Placeholder]"}
    ...
    async def _log(
        self, org_id: Optional[uuid.UUID], execution_id: uuid.UUID,
        step_key: Optional[str], level: str, message: str
    ) -> None:
        await self.repo.create_log(org_id, {
    ...
    async def _fail_execution(
        self, execution: WorkflowExecution, org_id: Optional[uuid.UUID], error: str
    ) -> WorkflowExecution:
        exec_obj = await self.repo.update_execution(execution, {
            "status": "failed",
            "end_time": datetime.now(UTC),
            "error_message": error,
            "duration_ms": self._elapsed_ms(execution.start_time),
        })
        await self._log(org_id, execution.id, None, "ERROR", f"Execution failed: {error}")
        return exec_obj
