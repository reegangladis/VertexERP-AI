...
class RAGRetrievalService:
    ...
    async def retrieve(
        ...
        for res in vector_results:
            ...
            if score >= min_score:
                chunk_uuid = uuid.UUID(res["vector_id"]) if self._is_valid_uuid(res["vector_id"]) else uuid.uuid4()
                doc_uuid = uuid.UUID(str(meta.get("document_id"))) if meta.get("document_id") and self._is_valid_uuid(str(meta.get("document_id"))) else uuid.uuid4()

                results.append({
                    "chunk_id": chunk_uuid,
                    "document_id": doc_uuid,
                    "document_title": meta.get("document_title", "Indexed Enterprise Document"),
