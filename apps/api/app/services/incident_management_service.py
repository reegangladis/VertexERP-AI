...
class IncidentManagementService:
    ...
    def resolve_incident(
        self, incident_number: str, mttr_minutes: float, root_cause: str
    ) -> dict[str, Any]:
        """Marks an operational incident as RESOLVED and records final MTTR."""
        inc = next(
            (i for i in self._incidents if i["incident_number"] == incident_number),
            None,
        )
        if not inc:
    ...
    def get_mttr_summary(self) -> dict[str, Any]:
        ...
        avg_mttr = (
            sum(i["mttr_minutes"] for i in resolved) / len(resolved)
            if resolved
            else 0.0
        )
        ...
        return {
            ...
            "open_incidents": len(
                [i for i in self._incidents if i["status"] != "RESOLVED"]
            ),
