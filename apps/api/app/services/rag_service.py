...
class RAGService:
    ...
    async def create_collection(
        self, org_id: uuid.UUID, user_id: uuid.UUID, schema: KnowledgeCollectionCreate
    ) -> KnowledgeCollection:
    ...
    async def get_collection(
        self, collection_id: uuid.UUID, org_id: uuid.UUID
    ...
    async def upload_document(
        ...
        for sc in saved_chunks:
            vector = await self.embedding_service.get_embedding(
                text=sc.content, provider="openai"
            )
            vector_id = str(sc.id)
            ...
            await self.vector_db_service.insert_vector(
                ...
                metadata={
                    ...
                    "collection_id": (
                        str(created_doc.collection_id)
                        if created_doc.collection_id
                        else None
                    ),
    ...
    async def create_chat_session(
        self, org_id: uuid.UUID, user_id: uuid.UUID, title: str = "New Conversation"
    ) -> RAGChatSession:
        session = RAGChatSession(
            organization_id=org_id, user_id=user_id, title=title, is_pinned=False
        )
        return await self.repository.create_chat_session(session)

    async def get_chat_session(
        self, session_id: uuid.UUID, org_id: uuid.UUID, user_id: uuid.UUID
    ) -> RAGChatSession | None:
        return await self.repository.get_chat_session(session_id, org_id, user_id)

    async def list_chat_sessions(
        self, org_id: uuid.UUID, user_id: uuid.UUID
    ) -> Sequence[RAGChatSession]:
    ...
    async def send_chat_message(
        ...
        user_message = RAGChatMessage(session_id=session_id, role="user", content=query)
    ...
    async def submit_feedback(
        self, org_id: uuid.UUID, user_id: uuid.UUID, schema: FeedbackCreate
    ) -> RAGFeedback:
