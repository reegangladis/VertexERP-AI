...
class FinanceService:
    ...
    async def update_account(
        ...
        await self.log_audit(
            ...
            None,
            "UPDATE_ACCOUNT",
            {"account_id": str(account_id), "code": account.account_code},
        )
        return updated
    ...
    async def delete_account(self, account_id: uuid.UUID) -> bool:
        ...
        if account.is_system:
            raise HTTPException(
                status_code=400, detail="Cannot delete a system-defined account."
            )

        # Check if ledger entries exist for this account
        ledgers = await self.ledger_repo.get_by_account(
            account.organization_id, account_id
        )
        if ledgers:
            raise HTTPException(
                status_code=400,
                detail="Cannot delete account with existing ledger entries.",
            )
    ...
    async def create_fiscal_period(
        self, org_id: uuid.UUID, data: FiscalPeriodCreate
    ) -> FiscalPeriod:
        period = FiscalPeriod(
            organization_id=org_id,
    ...
    async def close_fiscal_period(self, period_id: uuid.UUID) -> FiscalPeriod:
        ...
        return await self.period_repo.update(
            period, {"is_closed": True, "closed_at": datetime.utcnow()}
        )

    # --- 3. JOURNAL ENTRIES & LEDGER POSTING ---
    async def create_journal_entry(
        self, org_id: uuid.UUID, data: JournalEntryCreate
    ) -> JournalEntry:
        ...
        if total_debit <= 0:
            raise HTTPException(
                status_code=400,
                detail="Journal entry lines must have a non-zero debit/credit amount.",
            )

        entry_num = (
            data.entry_number or f"JE-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        )
    ...
    async def post_journal_entry(
        ...
        for line in entry.lines:
            ...
            if account.account_type in ["Assets", "Expenses"]:
                account.balance = (
                    float(account.balance) + float(line.debit) - float(line.credit)
                )
            else:
                account.balance = (
                    float(account.balance) + float(line.credit) - float(line.debit)
                )
    ...
    async def reverse_journal_entry(
        ...
        if not original or original.status != "POSTED":
            raise HTTPException(
                status_code=400, detail="Only posted journal entries can be reversed."
            )
        ...
        reversal_entry = await self.create_journal_entry(
            original.organization_id, reversal_create
        )
        ...
        await self.log_audit(
            ...
            {"original_id": str(entry_id), "reversal_id": str(posted_reversal.id)},
        )
    ...
    async def create_invoice(
        self, org_id: uuid.UUID, data: CustomerInvoiceCreate
    ) -> CustomerInvoice:
        inv_num = (
            data.invoice_number or f"INV-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        )
        ...
        for item in data.items:
            ...
            if item.tax_rate_id and item.tax_rate_id in tax_rates_map:
                item_tax = round(
                    line_val * (tax_rates_map[item.tax_rate_id] / 100.0), 2
                )
        ...
        rev_acct = next(
            (a for a in accounts if a.account_code in ["4000", "4100"]), accounts[-1]
        )
        ...
        if tax_total > 0 and tax_acct:
            lines.append(
                JournalEntryLineCreate(
                    account_id=rev_acct.id, debit=0.0, credit=round(subtotal, 2)
                )
            )
            lines.append(
                JournalEntryLineCreate(
                    account_id=tax_acct.id, debit=0.0, credit=round(tax_total, 2)
                )
            )
        else:
            lines.append(
                JournalEntryLineCreate(
                    account_id=rev_acct.id, debit=0.0, credit=total_amount
                )
            )
    ...
    async def create_credit_note(
        self, org_id: uuid.UUID, data: CreditNoteCreate
    ) -> CreditNote:
        ...
        je_data = JournalEntryCreate(
            ...
            lines=[
                JournalEntryLineCreate(
                    account_id=rev_acct.id, debit=data.amount, credit=0.0
                ),
                JournalEntryLineCreate(
                    account_id=ar_acct.id,
                    debit=0.0,
        ...
        await self.log_audit(
            ...
            "CREATE_CREDIT_NOTE",
            {
                "credit_note_id": str(saved_cn.id),
                "number": cn_num,
                "amount": data.amount,
            },
        )
    ...
    async def create_bill(
        self, org_id: uuid.UUID, data: SupplierBillCreate
    ) -> SupplierBill:
        bill_num = (
            data.bill_number or f"BILL-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        )
        ...
        je_data = JournalEntryCreate(
            ...
            lines=[
                JournalEntryLineCreate(
                    account_id=exp_acct.id, debit=total_amount, credit=0.0
                ),
                JournalEntryLineCreate(
        ...
        await self.log_audit(
            ...
            None,
            "CREATE_BILL",
            {"bill_id": str(saved_bill.id), "number": bill_num, "amount": total_amount},
        )
    ...
    async def create_debit_note(
        self, org_id: uuid.UUID, data: DebitNoteCreate
    ) -> DebitNote:
        ...
        je_data = JournalEntryCreate(
            ...
            lines=[
                ...
                JournalEntryLineCreate(
                    account_id=exp_acct.id, debit=0.0, credit=data.amount
                ),
            ],
        )
        je = await self.create_journal_entry(org_id, je_data)
    ...
    async def create_payment(self, org_id: uuid.UUID, data: PaymentCreate) -> Payment:
        ...
        if data.bank_account_id:
            ...
            if bank_acct:
                ...
                if tx_type == "DEPOSIT":
                    bank_acct.current_balance = float(
                        bank_acct.current_balance
                    ) + float(data.amount)
                else:
                    bank_acct.current_balance = float(
                        bank_acct.current_balance
        ...
        accounts = await self.get_accounts(org_id)
        cash_bank_acct = next(
            (a for a in accounts if a.account_code == "1020"), accounts[0]
        )
        ...
        if data.payment_type == "RECEIPT" and data.invoice_id:
            ...
            if inv:
                ...
                ar_acct = next(
                    (a for a in accounts if a.account_code == "1100"), accounts[0]
                )
                je_data = JournalEntryCreate(
                    ...
                    lines=[
                        JournalEntryLineCreate(
                            account_id=cash_bank_acct.id, debit=data.amount, credit=0.0
                        ),
                        JournalEntryLineCreate(
                            account_id=ar_acct.id,
                            debit=0.0,
            ...
            if bill:
                ...
                ap_acct = next(
                    (a for a in accounts if a.account_code == "2000"), accounts[0]
                )
                je_data = JournalEntryCreate(
                    entry_number=f"JE-{pay_num}",
                    entry_date=data.payment_date,
                    ...
                    lines=[
                        ...
                        JournalEntryLineCreate(
                            account_id=cash_bank_acct.id, debit=0.0, credit=data.amount
                        ),
                    ],
                )
                je = await self.create_journal_entry(org_id, je_data)
    ...
    async def create_bank_account(
        self, org_id: uuid.UUID, data: BankAccountCreate
    ) -> BankAccount:
        account = BankAccount(
            organization_id=org_id,
            account_name=data.account_name,
    ...
    async def create_bank_transaction(
        self, data: BankTransactionCreate
    ) -> BankTransaction:
        bank_acct = await self.bank_repo.get_by_id(data.bank_account_id)
        if not bank_acct:
        ...
        if data.transaction_type in ["DEPOSIT"]:
            bank_acct.current_balance = float(bank_acct.current_balance) + float(
                data.amount
            )
    ...
    async def reconcile_bank_account(
        self, data: ReconciliationCreate
    ) -> Reconciliation:
        bank_acct = await self.bank_repo.get_by_id(data.bank_account_id)
        if not bank_acct:
            raise HTTPException(status_code=404, detail="Bank account not found.")
    ...
    async def create_expense_category(
        self, org_id: uuid.UUID, data: ExpenseCategoryCreate
    ) -> ExpenseCategory:
        cat = ExpenseCategory(
            organization_id=org_id,
            name=data.name,
    ...
    async def get_expense_categories(self, org_id: uuid.UUID) -> list[ExpenseCategory]:
        ...
        if not cats:
            default_cat = ExpenseCategory(
                organization_id=org_id, name="Travel & Meals", code="EXP-TRV"
            )
            cats.append(await self.expense_cat_repo.create(default_cat))
        return cats

    async def create_expense_claim(
        self, org_id: uuid.UUID, data: ExpenseClaimCreate
    ) -> ExpenseClaim:
    ...
    async def approve_expense_claim(
        self, claim_id: uuid.UUID, user_id: uuid.UUID
    ) -> ExpenseClaim:
    ...
    async def reimburse_expense_claim(self, claim_id: uuid.UUID) -> ExpenseClaim:
        ...
        if not claim or claim.status != "APPROVED":
            raise HTTPException(
                status_code=400,
                detail="Only approved expense claims can be reimbursed.",
            )
        ...
        je_data = JournalEntryCreate(
            ...
            lines=[
                JournalEntryLineCreate(
                    account_id=exp_acct.id, debit=claim.amount, credit=0.0
                ),
                JournalEntryLineCreate(
                    account_id=cash_acct.id, debit=0.0, credit=claim.amount
                ),
            ],
        )
    ...
    async def create_tax_profile(
        self, org_id: uuid.UUID, data: TaxProfileCreate
    ) -> TaxProfile:
        profile = TaxProfile(
            organization_id=org_id,
            name=data.name,
    ...
    async def get_tax_profiles(self, org_id: uuid.UUID) -> list[TaxProfile]:
        ...
        if not profiles:
            def_profile = TaxProfile(
                organization_id=org_id,
                name="Standard US Tax Profile",
                country="US",
                is_default=True,
            )
    ...
    async def create_asset_category(
        self, org_id: uuid.UUID, data: AssetCategoryCreate
    ) -> AssetCategory:
        cat = AssetCategory(
            organization_id=org_id,
            name=data.name,
    ...
    async def create_fixed_asset(
        self, org_id: uuid.UUID, data: FixedAssetCreate
    ) -> FixedAsset:
    ...
    async def dispose_fixed_asset(
        self, asset_id: uuid.UUID, disposal_amount: float
    ) -> FixedAsset:
    ...
    async def get_trial_balance(
        self, org_id: uuid.UUID, as_of: date | None = None
    ) -> TrialBalanceReportResponse:
    ...
    async def get_aging_report(
        self, org_id: uuid.UUID, report_type: str = "RECEIVABLE"
    ) -> AgingReportResponse:
        buckets = []
        tot_outstanding = 0.0
    ...
    async def search_finance(
        ...
        for je in journals:
            if q in je.entry_number.lower() or (
                je.reference and q in je.reference.lower()
            ):
                results.append(
                    FinanceSearchResult(
    ...
    async def get_tax_report(
        ...
        tot_tax_collected = sum(
            float(inv.tax_total) for inv in invoices if inv.issue_date <= as_of_date
        )
        tot_tax_paid = sum(
            float(b.tax_total) for b in bills if b.bill_date <= as_of_date
        )
            ...
            for r in getattr(p, "rates", []):
                items.append(
                    TaxReportItem(
                        ...
                        taxable_amount=(
                            round(tot_tax_collected * 10, 2) if r.type == "VAT" else 0.0
                        ),
    ...
    async def get_budget_report(
        self, org_id: uuid.UUID, fiscal_year: int | None = None
    ) -> BudgetReportResponse:
    ...
    async def get_dashboard_summary(self, org_id: uuid.UUID) -> FinanceDashboardSummary:
        ...
        tot_ar = sum(
            float(inv.total_amount) - float(inv.paid_amount) for inv in invoices
        )
        ...
        utilization = round(
            (pl.total_expenses / tot_budgeted * 100.0) if tot_budgeted > 0 else 0.0, 2
        )
