...
class MLOpsService:
    ...
    async def create_deployment(
        self, organization_id: uuid.UUID, payload: MLDeploymentCreate, username: str
    ) -> MLDeployment:
    ...
    async def update_traffic(
        self, deployment_id: uuid.UUID, traffic_pct: float, username: str
    ) -> MLDeployment | None:
        ...
        updated_dep = await self.repo.update_deployment(
            deployment_id, {"target_traffic_percentage": traffic_pct}
        )
    ...
    async def rollback_deployment(
        ...
        previous_version_id = dep.model_version_id
        updated_dep = await self.repo.update_deployment(
            deployment_id, {"model_version_id": target_version_id, "status": "ACTIVE"}
        )
        ...
        history = DeploymentHistory(
            ...
            triggered_by=username,
            notes=notes
            or f"Deployment rolled back to previous stable model version ID: {target_version_id}.",
        )
        await self.repo.create_history(history)
        return updated_dep
    ...
    async def create_pipeline_template(
        self, organization_id: uuid.UUID, payload: PipelineTemplateCreate
    ) -> PipelineTemplate:
    ...
    async def run_pipeline(
        self, organization_id: uuid.UUID, payload: PipelineRunCreate
    ) -> PipelineRun:
        ...
        logs += "[INFO] Fetching pipeline template config...\n"
        logs += "[INFO] Commencing Continuous Integration validation checks...\n"

        # Check standard validations
        validation_passed = True
        if random.random() < 0.05:  # 5% chance of failing validation
            validation_passed = False
            logs += "[ERROR] Artifact validation failed: checksum verification error.\n"
            logs += "[ERROR] Pipeline execution terminated due to errors.\n"
            status = "FAILED"
            metrics = {"validation_passed": False, "accuracy": 0.0}
        else:
            logs += "[INFO] Artifact validation check: SUCCESS.\n"
            logs += "[INFO] Running security signature scan...\n"
            logs += "[INFO] Vulnerability check: No threat detected. SUCCESS.\n"
            logs += (
                "[INFO] Loading datasets and training adaptation configurations...\n"
            )
            logs += "[INFO] Epoch 1/5: loss=0.45, accuracy=0.81\n"
            logs += "[INFO] Epoch 5/5: loss=0.12, accuracy=0.96\n"
            logs += "[INFO] Running Continuous Validation checks against evaluation baseline...\n"
            logs += "[INFO] Promotion checks: Model validation passed successfully.\n"
            logs += "[INFO] Pipeline Run completed successfully.\n"
            status = "COMPLETED"
            metrics = {
                "validation_passed": True,
    ...
    async def request_approval(
        self, organization_id: uuid.UUID, payload: ModelApprovalCreate
    ) -> ModelApproval:
    ...
    async def decide_approval(
        self, approval_id: uuid.UUID, payload: ModelApprovalUpdate
    ) -> ModelApproval | None:
    ...
    async def get_metrics(
        ...
        return await self.repo.get_monitoring_metrics(
            organization_id, deployment_id, limit
        )
    ...
    async def trigger_retraining_job(
        self, organization_id: uuid.UUID, payload: RetrainingJobCreate
    ) -> RetrainingJob:
        job = RetrainingJob(
            organization_id=organization_id,
            model_id=payload.model_id,
