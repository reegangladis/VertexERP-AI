...
class UserService(BaseService[User, UserRepository]):
    def __init__(self, repository: UserRepository, password_history_repo: PasswordHistoryRepository):
    ...
    async def update_password(self, user: User, new_password: str) -> User:
        ...
        for old_pwd in history[:3]:
            if verify_password(new_password, old_pwd.password_hash):
                raise ValueError("Password matches one of your last 3 passwords. Please choose another.")
    ...
    async def increment_failed_attempts(self, user: User, threshold: int = 5, duration_minutes: int = 15) -> User:
