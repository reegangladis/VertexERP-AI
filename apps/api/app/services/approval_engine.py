...
from app.models.workflow import ApprovalHistory, ApprovalRequest
...
class ApprovalEngine:
    ...
    async def create_approval_request(
        ...
        obj = await self.repo.create_approval(
            org_id,
            {
        ...
        await self._record_history(
            org_id, obj.id, "created", data.requester_id, "Approval request created."
        )
    ...
    async def process_action(
        ...
        if approval.status not in ("pending", "delegated"):
            raise ValueError(
                f"Cannot process action on approval with status: {approval.status}"
            )
        ...
        if action == "approve":
            ...
            await self.repo.update_approval(
                approval, {"approver_id": payload.delegate_to_user_id}
            )
            ...
            await self.repo.update_approval(
                approval, {"approver_id": escalate_to, "level": approval.level + 1}
            )
    ...
    async def _handle_approve(
        self, org_id: uuid.UUID | None, approval: ApprovalRequest
    ) -> str:
    ...
    async def check_sla_escalations(
        ...
        for approval in approvals:
            if (
                approval.due_date
                and approval.due_date < now
                and approval.escalation_user_id
            ):
                await self.repo.update_approval(
                    approval,
                    {
    ...
    async def list_pending_for_approver(
        ...
        return await self.repo.list_approvals(
            org_id, approver_id=approver_id, status="pending"
        )
    ...
    async def _record_history(
        ...
        return await self.repo.create_approval_history(
            org_id,
            {
