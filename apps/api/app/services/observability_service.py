...
class ObservabilityService:
    ...
    async def record_metric(
        self, organization_id: Optional[uuid.UUID], metric_in: SystemMetricCreate
    ) -> SystemMetric:
        metric = SystemMetric(
    ...
    # =========================================================================
    async def write_log(
        self, organization_id: Optional[uuid.UUID], log_in: ApplicationLogCreate
    ) -> ApplicationLog:
        log = ApplicationLog(
            organization_id=organization_id,
    ...
    async def record_trace_span(
        self, organization_id: Optional[uuid.UUID], span_in: TraceSpanCreate
    ) -> Trace:
        span = Trace(
            organization_id=organization_id,
    ...
    async def get_dependency_map(
        ...
        if not deps:
            ...
            deps = [
                {"caller": "Web Portal", "callee": "API Gateway", "call_count": 14200, "avg_duration_ms": 12.5, "error_rate": 0.0},
                {"caller": "API Gateway", "callee": "Authentication Service", "call_count": 4200, "avg_duration_ms": 45.2, "error_rate": 0.01},
                {"caller": "API Gateway", "callee": "HR Service", "call_count": 2100, "avg_duration_ms": 82.4, "error_rate": 0.0},
                {"caller": "API Gateway", "callee": "Finance Service", "call_count": 3200, "avg_duration_ms": 115.0, "error_rate": 0.02},
                {"caller": "API Gateway", "callee": "AI Copilot Platform", "call_count": 1500, "avg_duration_ms": 1840.0, "error_rate": 0.04},
                {"caller": "AI Copilot Platform", "callee": "RAG Intelligence Service", "call_count": 1200, "avg_duration_ms": 620.0, "error_rate": 0.0},
                {"caller": "RAG Intelligence Service", "callee": "Vector Database (PGVector)", "call_count": 2400, "avg_duration_ms": 42.1, "error_rate": 0.0},
            ]
        return deps
    ...
    async def create_alert(
        self, organization_id: Optional[uuid.UUID], alert_in: AlertCreate
    ) -> Alert:
        alert = Alert(
            organization_id=organization_id,
    ...
    async def evaluate_metric_thresholds(
        self, organization_id: Optional[uuid.UUID], metric: SystemMetric
    ) -> None:
        ...
        if metric.metric_name == "cpu_usage" and metric.value > 85.0:
            ...
            breached = True
            description = f"API Latency latency breached 2000ms (Value: {metric.value}ms)"
            severity = "warning"
            
        if breached:
            # Check if active alert already exists to prevent duplication
            existing_alerts = await self.repo.get_alerts(organization_id, status="active")
            duplicate = any(
                a.rule_name == f"Threshold Breach: {metric.metric_name}" for a in existing_alerts
            )
            if not duplicate:
    ...
    async def update_alert(
        ...
        if organization_id and alert.organization_id and alert.organization_id != organization_id:
            return None
    ...
    # =========================================================================
    async def get_system_health(self, organization_id: Optional[uuid.UUID]) -> Dict[str, Any]:
        ...
        services_list = [
            {"name": "ERP Core Services", "type": "ERP Services", "status": "healthy", "liveness": True, "readiness": True, "uptime": 1209600.0, "latency": 15.4, "deps": {"postgres": db_status}},
            {"name": "REST API Gateway", "type": "REST APIs", "status": "healthy", "liveness": True, "readiness": True, "uptime": 2419200.0, "latency": 8.2, "deps": {"redis": redis_status}},
            {"name": "Database cluster", "type": "Databases", "status": db_status, "liveness": db_status == "healthy", "readiness": db_status == "healthy", "uptime": 3628800.0, "latency": db_latency, "deps": {}},
            {"name": "Identity Provider (MFA)", "type": "Authentication", "status": "healthy", "liveness": True, "readiness": True, "uptime": 2419200.0, "latency": 42.8, "deps": {"redis": redis_status}},
            {"name": "ETL Pipelines Executor", "type": "ETL Pipelines", "status": "healthy", "liveness": True, "readiness": True, "uptime": 604800.0, "latency": 120.5, "deps": {"postgres": db_status}},
            {"name": "ML Training & Registry", "type": "Machine Learning", "status": "healthy", "liveness": True, "readiness": True, "uptime": 604800.0, "latency": 250.0, "deps": {"postgres": db_status}},
            {"name": "RAG Embedding Engines", "type": "RAG", "status": "healthy", "liveness": True, "readiness": True, "uptime": 1209600.0, "latency": 55.4, "deps": {"postgres": db_status}},
            {"name": "AI Copilot Core", "type": "AI Copilot", "status": "healthy", "liveness": True, "readiness": True, "uptime": 1209600.0, "latency": 1800.0, "deps": {"redis": redis_status}},
        ]
    ...
    async def record_event(
        self, organization_id: Optional[uuid.UUID], event_in: ObservabilityEventCreate
    ) -> ObservabilityEvent:
    ...
    async def get_events(
        self, organization_id: Optional[uuid.UUID], limit: int = 50
    ) -> List[ObservabilityEvent]:
        events = await self.repo.get_events(organization_id, limit=limit)
        if not events:
            # Seed mock recent audit events
            mock_data = [
                ObservabilityEventCreate(event_type="deployment", name="Deployed version 1.2.0-rc3", description="Production deployment of the analytics package.", severity="info"),
                ObservabilityEventCreate(event_type="migration", name="Database schema check", description="Alembic check passed successfully.", severity="info"),
                ObservabilityEventCreate(event_type="security_audit", name="MFA enforce verification", description="Verified tenant configuration isolation parameters.", severity="info"),
            ]
            for m in mock_data:
