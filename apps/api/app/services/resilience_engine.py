    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == "OPEN":
            if now - self.last_failure_time > self.recovery_timeout_seconds:
                ...
                raise CircuitBreakerOpenException("Circuit Breaker is OPEN. Request rejected to prevent cascading failure.")
...
class Bulkhead:
    ...
    def __enter__(self):
        if self.current_calls >= self.max_concurrent_calls:
            raise RuntimeError("Bulkhead capacity reached. Request rejected to isolate resource pool.")
