...
class ManufacturingService:
    ...
    async def create_bom(
        self, organization_id: uuid.UUID, data: BOMCreate, user_id: Optional[uuid.UUID] = None
    ) -> BillOfMaterial:
        total_cost = 0.0
        bom = BillOfMaterial(
            organization_id=organization_id,
        ...
        await self._log_audit("mfg.bom.created", organization_id, user_id, {"bom_id": str(bom.id), "code": bom.code})
        await self.db.commit()
        return await self.bom_repo.get_with_items(bom.id)

    async def update_bom(
        self, bom_id: uuid.UUID, data: BOMUpdate, user_id: Optional[uuid.UUID] = None
    ) -> BillOfMaterial:
        ...
        await self._log_audit("mfg.bom.updated", bom.organization_id, user_id, {"bom_id": str(bom.id), "code": bom.code})
        await self.db.commit()
        return await self.bom_repo.get_with_items(bom.id)

    async def delete_bom(self, bom_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        bom = await self.bom_repo.get(bom_id)
        if not bom:
            return False
        bom.is_deleted = True
        bom.deleted_at = datetime.utcnow()
        await self._log_audit("mfg.bom.deleted", bom.organization_id, user_id, {"bom_id": str(bom.id)})
        await self.db.commit()
        return True
    ...
    async def approve_bom(self, bom_id: uuid.UUID, user_id: uuid.UUID) -> BillOfMaterial:
        ...
        await self._log_audit("mfg.bom.approved", bom.organization_id, user_id, {"bom_id": str(bom.id)})
        await self.db.commit()
        return await self.bom_repo.get_with_items(bom.id)
    ...
    async def create_routing(
        self, organization_id: uuid.UUID, data: RoutingCreate, user_id: Optional[uuid.UUID] = None
    ) -> Routing:
        total_time = 0.0
        routing = Routing(
            organization_id=organization_id,
        ...
        await self._log_audit("mfg.routing.created", organization_id, user_id, {"routing_id": str(routing.id)})
        await self.db.commit()
        return await self.routing_repo.get_with_operations(routing.id)

    async def update_routing(
        self, routing_id: uuid.UUID, data: RoutingUpdate, user_id: Optional[uuid.UUID] = None
    ) -> Routing:
        ...
        await self._log_audit("mfg.routing.updated", routing.organization_id, user_id, {"routing_id": str(routing.id)})
        await self.db.commit()
        return await self.routing_repo.get_with_operations(routing.id)

    async def delete_routing(self, routing_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        routing = await self.routing_repo.get(routing_id)
        if not routing:
            return False
        routing.is_deleted = True
        routing.deleted_at = datetime.utcnow()
        await self._log_audit("mfg.routing.deleted", routing.organization_id, user_id, {"routing_id": str(routing.id)})
        await self.db.commit()
        return True

    # --- Work Centers & Machines ---
    async def create_work_center(
        self, organization_id: uuid.UUID, data: WorkCenterCreate, user_id: Optional[uuid.UUID] = None
    ) -> WorkCenter:
        wc = WorkCenter(
            organization_id=organization_id,
            code=data.code,
    ...
    async def update_work_center(
        self, wc_id: uuid.UUID, data: WorkCenterUpdate, user_id: Optional[uuid.UUID] = None
    ) -> WorkCenter:
    ...
    async def delete_work_center(self, wc_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        wc = await self.work_center_repo.get(wc_id)
        if not wc:
            return False
        wc.is_deleted = True
    ...
    async def create_machine(
        self, data: MachineCreate, user_id: Optional[uuid.UUID] = None
    ) -> Machine:
        m = Machine(
    ...
    async def update_machine(
        self, machine_id: uuid.UUID, data: MachineUpdate, user_id: Optional[uuid.UUID] = None
    ) -> Machine:
    ...
    async def delete_machine(self, machine_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        m = await self.machine_repo.get(machine_id)
        if not m:
            return False
        m.is_deleted = True
    ...
    async def log_machine_downtime(
        self, data: MachineDowntimeCreate, user_id: Optional[uuid.UUID] = None
    ) -> MachineDowntime:
        duration = 0.0
        if data.end_time and data.start_time:
            duration = max(0.0, (data.end_time - data.start_time).total_seconds() / 60.0)
        ...
        if m:
            m.status = "BREAKDOWN" if data.reason_category == "UNPLANNED_BREAKDOWN" else "MAINTENANCE"
    ...
    async def create_production_order(
        self, organization_id: uuid.UUID, data: ProductionOrderCreate, user_id: Optional[uuid.UUID] = None
    ) -> ProductionOrder:
        ...
        await self._log_audit("mfg.production_order.created", organization_id, user_id, {"order_id": str(order.id), "number": order.order_number})
        await self.db.commit()
        return await self.order_repo.get_with_items(order.id)

    async def update_production_order(
        self, order_id: uuid.UUID, data: ProductionOrderUpdate, user_id: Optional[uuid.UUID] = None
    ) -> ProductionOrder:
        ...
        await self._log_audit("mfg.production_order.updated", order.organization_id, user_id, {"order_id": str(order.id), "status": order.status})
        await self.db.commit()
        return await self.order_repo.get_with_items(order.id)

    async def delete_production_order(self, order_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        order = await self.order_repo.get(order_id)
        if not order:
            return False
        order.is_deleted = True
        order.deleted_at = datetime.utcnow()
        await self._log_audit("mfg.production_order.deleted", order.organization_id, user_id, {"order_id": str(order.id)})
        await self.db.commit()
        return True
    ...
    async def reserve_materials(
        ...
        for item in bom.items:
            req_qty = item.quantity * (order.planned_quantity / bom.base_quantity if bom.base_quantity > 0 else 1.0)
            req_qty *= (1 + (item.scrap_factor_percent / 100.0))

            stock_stmt = select(func.coalesce(func.sum(StockLevel.quantity_on_hand), 0.0)).where(
                StockLevel.product_id == item.component_product_id
            )
        ...
        res_status = "FULL" if len(shortages) == 0 else ("PARTIAL" if len(allocated) > 0 else "NOT_RESERVED")
        order.material_reservation_status = res_status
        await self._log_audit("mfg.materials.reserved", order.organization_id, user_id, {"order_id": str(order.id), "status": res_status})
        await self.db.commit()
    ...
    async def update_production_order_item(
        self, item_id: uuid.UUID, data: ProductionOrderItemUpdate, user_id: Optional[uuid.UUID] = None
    ) -> ProductionOrderItem:
    ...
    async def calculate_production_order_costs(self, order_id: uuid.UUID) -> ProductionCostSummaryResponse:
        order = await self.order_repo.get_with_items(order_id)
        if not order:
            raise ValueError("Production Order not found")
        ...
        if consumptions:
            for c in consumptions:
                material_cost += c.consumed_quantity * c.unit_cost if c.consumed_quantity > 0 else c.reserved_quantity * c.unit_cost
        elif order.bom_id:
            bom = await self.bom_repo.get_with_items(order.bom_id)
            if bom:
                scale = (order.planned_quantity / bom.base_quantity) if bom.base_quantity > 0 else 1.0
        ...
        if order.items:
            for item in order.items:
                ...
                hours = item.actual_hours if item.actual_hours > 0 else item.planned_hours
        ...
        unit_actual = (total_actual / order.completed_quantity) if order.completed_quantity > 0 else (total_actual / order.planned_quantity if order.planned_quantity > 0 else 0.0)
        
        estimated_total = total_actual * 0.95  # baseline estimation calculation
        variance = total_actual - estimated_total
        variance_pct = (variance / estimated_total * 100.0) if estimated_total > 0 else 0.0

        await self._log_audit("mfg.costing.calculated", order.organization_id, None, {"order_id": str(order.id), "total_cost": total_actual})
        await self.db.commit()
    ...
    async def log_production_output(
        self, data: ProductionLogCreate, user_id: Optional[uuid.UUID] = None
    ) -> ProductionLog:
        log = ProductionLog(
            production_order_id=data.production_order_id,
            work_center_id=data.work_center_id,
        ...
        await self._log_audit("mfg.shop_floor.logged", order.organization_id if order else None, user_id, {"log_id": str(log.id), "qty": data.quantity_produced})
        await self.db.commit()
    ...
    async def create_quality_inspection(
        self, organization_id: uuid.UUID, data: QualityInspectionCreate, user_id: Optional[uuid.UUID] = None
    ) -> QualityInspection:
        ...
        await self._log_audit("mfg.quality.inspected", organization_id, user_id, {"inspection_id": str(insp.id), "decision": insp.decision})
        await self.db.commit()
        return await self.inspection_repo.get_with_results(insp.id)

    async def update_quality_inspection(
        self, inspection_id: uuid.UUID, data: QualityInspectionUpdate, user_id: Optional[uuid.UUID] = None
    ) -> QualityInspection:
        ...
        await self._log_audit("mfg.quality.updated", insp.organization_id, user_id, {"inspection_id": str(insp.id), "decision": insp.decision})
        await self.db.commit()
        return await self.inspection_repo.get_with_results(insp.id)

    async def delete_quality_inspection(self, inspection_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
    ...
    async def create_maintenance_request(
        self, organization_id: uuid.UUID, data: MaintenanceRequestCreate, user_id: Optional[uuid.UUID] = None
    ) -> MaintenanceRequest:
        ...
        await self._log_audit("mfg.maintenance.created", organization_id, user_id, {"request_id": str(req.id), "ticket": req.ticket_number})
        await self.db.commit()
        await self.db.refresh(req)
        return req

    async def update_maintenance_request(
        self, request_id: uuid.UUID, data: MaintenanceRequestUpdate, user_id: Optional[uuid.UUID] = None
    ) -> MaintenanceRequest:
        ...
        await self._log_audit("mfg.maintenance.updated", req.organization_id, user_id, {"request_id": str(req.id), "status": req.status})
        await self.db.commit()
        await self.db.refresh(req)
        return req

    async def delete_maintenance_request(self, request_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> bool:
        req = await self.maintenance_repo.get(request_id)
        if not req:
            return False
        req.is_deleted = True
    ...
    async def log_maintenance_work(
        self, data: MaintenanceLogCreate, user_id: Optional[uuid.UUID] = None
    ) -> MaintenanceLog:
        mlog = MaintenanceLog(
        ...
        await self._log_audit("mfg.maintenance.logged", None, user_id, {"log_id": str(mlog.id), "machine_id": str(data.machine_id)})
        await self.db.commit()
    ...
    # --- MRP Engine ---
    async def run_mrp(
        self, organization_id: uuid.UUID, data: MRPRunCreate, user_id: Optional[uuid.UUID] = None
    ) -> MRPRun:
        ...
        self.db.add(mrp)
        await self._log_audit("mfg.mrp.executed", organization_id, user_id, {"run_number": mrp.run_number})
        await self.db.commit()
