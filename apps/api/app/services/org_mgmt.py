...
class BranchService(BaseService[Branch, BranchRepository]):
    ...
    async def validate_parent(
        self, branch_id: uuid.UUID, parent_id: uuid.UUID | None
    ) -> None:
        if not parent_id:
            return
        ...
        while curr_id:
            if curr_id in visited:
                raise OrgMgmtServiceException(
                    "Cyclic hierarchy detected in branch mappings."
                )
            visited.add(curr_id)
...
class DepartmentService(BaseService[Department, DepartmentRepository]):
    ...
    async def validate_parent(
        self, dept_id: uuid.UUID, parent_id: uuid.UUID | None
    ) -> None:
        if not parent_id:
            return
        ...
        while curr_id:
            if curr_id in visited:
                raise OrgMgmtServiceException(
                    "Cyclic hierarchy detected in department mappings."
                )
            visited.add(curr_id)
...
class TeamService(BaseService[Team, TeamRepository]):
    ...
    async def validate_parent(
        self, team_id: uuid.UUID, parent_id: uuid.UUID | None
    ) -> None:
        if not parent_id:
            return
        ...
        while curr_id:
            if curr_id in visited:
                raise OrgMgmtServiceException(
                    "Cyclic hierarchy detected in team mappings."
                )
            visited.add(curr_id)
...
class BusinessCalendarService(BaseService[BusinessCalendar, CalendarRepository]):
    ...
    async def configure_working_days(
        self, org_id: uuid.UUID, calendar_id: uuid.UUID, days_data: list[dict[str, Any]]
    ) -> list[WorkingDay]:
...
class MetadataService(BaseService[OrganizationMetadata, MetadataRepository]):
    ...
    async def set_metadata(
        self, org_id: uuid.UUID, key: str, value: str, value_type: str = "string"
    ) -> OrganizationMetadata:
        existing_list, _ = await self.repository.get_multi(
            filters={"organization_id": org_id, "key": key}
        )
        if existing_list:
            db_obj = existing_list[0]
            return await self.repository.update(
                db_obj, {"value": value, "value_type": value_type}
            )
        else:
