...
class RoleService(BaseService[Role, RoleRepository]):
    ...
    async def assign_permissions_to_role(self, role_id: uuid.UUID, permission_names: list[str]) -> Role | None:
    ...
    async def seed_default_roles(self) -> None:
        ...
        default_roles = [
            {"name": "Super Admin", "description": "Global administrator with full override permissions"},
            {"name": "Organization Admin", "description": "Tenant administrator managing an organization"},
            {"name": "Manager", "description": "Team lead overseeing users and metrics"},
