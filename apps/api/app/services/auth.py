...
from app.repositories.session import RefreshTokenRepository
...
class AuthService:
    ...
    async def register(
        ...
        existing_username = await self.user_service.get_by_username(
            user_data["username"]
        )
        ...
        admin_role = roles_map.get(
            "Organization Admin"
        ) or await self.role_repo.get_by_name("Organization Admin")
    ...
    async def login(
        ...
        if await self.user_service.is_account_locked(user):
            ...
            locked_until = (
                user.locked_until.strftime("%H:%M UTC") if user.locked_until else "soon"
            )
            raise HTTPException(
        ...
        await self.login_history_service.log_login(
            user.id, identifier, ip_address, user_agent, browser, os, "success"
        )
        await self.audit_service.log_action(
            user.id, user.organization_id, "user.login", ip_address, user_agent
        )

        logger.info(f"[AUTH] Successful login: {identifier} from {ip_address}")
    ...
    async def resend_verification_email(self, user_id: uuid.UUID) -> str | None:
    ...
    async def get_login_history(self, user_id: uuid.UUID, limit: int = 50) -> list:
