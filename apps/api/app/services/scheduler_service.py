...
class SchedulerService:
    ...
    # ─── CRUD ────────────────────────────────────────────────────────────────────
    async def create_job(self, org_id: Optional[uuid.UUID], data: ScheduledJobCreate) -> ScheduledJob:
        next_run = data.next_run_at or self._compute_next_run(data.cron_expression, data.schedule_type)
        job = await self.repo.create_scheduled_job(org_id, {
    ...
    async def update_job(
        self, org_id: Optional[uuid.UUID], job_id: uuid.UUID, data: ScheduledJobUpdate
        ...
        if "cron_expression" in updates and updates.get("cron_expression"):
            updates["next_run_at"] = self._compute_next_run(updates["cron_expression"], job.schedule_type)
        updated = await self.repo.update_scheduled_job(job, updates)
        await self.db.commit()
        return updated

    async def delete_job(self, org_id: Optional[uuid.UUID], job_id: uuid.UUID) -> bool:
        job = await self.repo.get_scheduled_job(org_id, job_id)
        if not job:
            return False
    ...
    async def list_jobs(
        ...
        return await self.repo.list_scheduled_jobs(org_id, status=status, skip=skip, limit=limit)

    async def get_job(self, org_id: Optional[uuid.UUID], job_id: uuid.UUID) -> Optional[ScheduledJob]:
        return await self.repo.get_scheduled_job(org_id, job_id)

    # ─── Manual Trigger ──────────────────────────────────────────────────────────
    async def trigger_now(self, org_id: Optional[uuid.UUID], job_id: uuid.UUID) -> Dict[str, Any]:
        ...
        await self.repo.update_scheduled_job(job, {
            "last_run_at": now,
            "next_run_at": next_run,
        })
        await self.db.commit()
    ...
    def _parse_cron_next(self, cron: str, after: datetime) -> Optional[datetime]:
        """
    ...
    def calculate_next_run(self, cron_expression: str) -> Optional[str]:
