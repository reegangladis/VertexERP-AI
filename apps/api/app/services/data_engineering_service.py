...
class DataEngineeringService:
    ...
    async def create_etl_job(
        self, organization_id: uuid.UUID, data: ETLJobCreate
    ) -> ETLJob:
        job = ETLJob(
    ...
    async def trigger_pipeline_run(self, job_id: uuid.UUID) -> ETLRun:
        ...
        await self.repo.add_pipeline_log(
            PipelineLog(
                ...
                details={
                    "null_checks": "PASSED",
                    "duplicates_dropped": extracted_cnt - transformed_cnt,
                },
            )
        )
    ...
    async def run_data_quality_inspection(
        ...
        rules = [
            {
                "rule_name": "NULL_CHECK_PRIMARY_KEYS",
                "rule_type": "NULL_CHECK",
                "status": "PASSED",
                "affected_rows": 0,
                "message": "Primary key null checks passed 100%",
            },
            {
                "rule_name": "DUPLICATE_CHECK_UNIQUENESS",
                "rule_type": "DUPLICATE_CHECK",
                "status": "PASSED",
                "affected_rows": 0,
                "message": "Zero duplicate records detected across entity IDs.",
            },
            {
                "rule_name": "SCHEMA_VALIDATION_TYPES",
                "rule_type": "SCHEMA_CHECK",
                "status": "PASSED",
                "affected_rows": 0,
                "message": "Data types match catalog definitions exactly.",
            },
            {
                "rule_name": "REFERENTIAL_INTEGRITY_FOREIGN_KEYS",
                "rule_type": "REFERENTIAL_CHECK",
                "status": "PASSED",
                "affected_rows": 0,
                "message": "All foreign key constraints resolved against dimension keys.",
            },
        ]
    ...
    async def get_quality_reports(
        ...
        if not reports:
            ...
            r2 = await self.run_data_quality_inspection(
                organization_id, "dim_customers"
            )
            reports = [r1, r2]
        return reports
    ...
    async def create_feature_group(
        self, organization_id: uuid.UUID, data: FeatureGroupCreate
    ) -> FeatureGroup:
        group = FeatureGroup(
            organization_id=organization_id,
        ...
        if not datasets:
            categories = [
                (
                    ...
                    "employee_dataset",
                    "EMPLOYEE",
                    "Workforce headcount, tenure, salary, performance ratings.",
                ),
                (
                    "Customer Dataset",
                    "customer_dataset",
                    "CUSTOMER",
                    "Customer account profiles, LTV, support ticket volumes, deal counts.",
                ),
                (
                    "Inventory Dataset",
                    "inventory_dataset",
                    "INVENTORY",
                    "Product stock levels, warehouse valuation, reorder status, aging schedule.",
                ),
                (
                    "Financial Dataset",
                    "financial_dataset",
                    "FINANCIAL",
                    "General ledger balances, cash flow metrics, AP/AR invoice summaries.",
                ),
                (
                    "Manufacturing Dataset",
                    "manufacturing_dataset",
                    "MANUFACTURING",
                    "Work center OEE, production yields, machine downtime, scrap counts.",
                ),
                (
                    "Sales Dataset",
                    "sales_dataset",
                    "SALES",
                    "Sales order line items, discount margins, customer acquisition velocity.",
                ),
                (
                    "Supplier Dataset",
                    "supplier_dataset",
                    "SUPPLIER",
                    "Supplier OTIF ratings, lead time variance, purchase order fulfillment.",
                ),
            ]
            datasets = []
    ...
    async def get_lineage_graph(
        ...
        nodes = [
            LineageNode(
                id="src_crm",
                label="CRM PostgreSQL Source",
                type="SOURCE_TABLE",
                category="Source Systems",
            ),
            LineageNode(
                id="src_hr",
                label="HR Database Source",
                type="SOURCE_TABLE",
                category="Source Systems",
            ),
            LineageNode(
                id="src_inv",
                label="Inventory DB Source",
                type="SOURCE_TABLE",
                category="Source Systems",
            ),
            LineageNode(
                id="lake_raw",
                label="Data Lake Raw Zone",
                type="RAW_ZONE",
                category="Data Lake",
            ),
            LineageNode(
                id="lake_proc",
                label="Data Lake Processed Zone",
                type="PROCESSED_ZONE",
                category="Data Lake",
            ),
            LineageNode(
                id="dw_dim_cust",
                label="DimCustomer (SCD2)",
                type="DIMENSION",
                category="Data Warehouse",
            ),
            LineageNode(
                id="dw_dim_emp",
                label="DimEmployee (SCD2)",
                type="DIMENSION",
                category="Data Warehouse",
            ),
            LineageNode(
            ...
            LineageNode(
                id="feature_churn",
                label="customer_churn_features",
                type="FEATURE",
                category="Feature Store",
            ),
            LineageNode(
                id="dataset_cust",
                label="Customer Dataset (Curated)",
                type="DATASET",
        ...
        edges = [
            LineageEdge(
                source="src_crm",
                target="lake_raw",
                label="Extract JSON/Parquet",
                type="EXTRACT",
            ),
            LineageEdge(
                source="src_hr",
                target="lake_raw",
                label="Extract CSV Streams",
                type="EXTRACT",
            ),
            LineageEdge(
                source="lake_raw",
                target="lake_proc",
                label="Schema Validation & Null Check",
                type="TRANSFORM",
            ),
            LineageEdge(
                source="lake_proc",
                target="dw_dim_cust",
                label="SCD Type 2 Dimension Load",
                type="LOAD",
            ),
            LineageEdge(
                source="lake_proc",
            ...
            LineageEdge(
                source="dw_dim_cust",
                target="dw_fact_sales",
                label="Star Schema Key Mapping",
                type="AGGREGATE",
            ),
            LineageEdge(
                source="dw_fact_sales",
                target="feature_churn",
                label="Feature Engineering SQL",
                type="FEATURE_GEN",
            ),
            LineageEdge(
                source="dw_fact_sales",
                target="dataset_cust",
                label="Curated Export",
                type="LOAD",
            ),
        ]
    ...
    async def get_data_lake_objects(
        ...
        if not objects:
            zones_data = [
                (
                    "RAW",
                    "s3://vertexerp-datalake/raw/crm/customers_20260726.parquet",
                    "PARQUET",
                    5242880,
                    12000,
                    "CRM",
                ...
                (
                    "PROCESSED",
                    "s3://vertexerp-datalake/processed/crm/customers_clean.parquet",
                    "PARQUET",
                    4194304,
                    11980,
                    "CRM",
                ...
                (
                    "CURATED",
                    "s3://vertexerp-datalake/curated/datasets/customer_dataset.json",
                    "JSON",
                    2097152,
                    10000,
                    "CRM",
                ...
                (
                    "ARCHIVE",
                    "s3://vertexerp-datalake/archive/2025/hr_attendance_old.parquet",
                    "PARQUET",
                    10485760,
                    50000,
                    "HR",
        ...
        if not records:
            mdm_samples = [
                ...
                (
                    ...
                    {
                        "sku": "PROD-A100",
                        "name": "Precision CNC Milling Bit",
                ...
                (
                    "SUPPLIER",
                    "GOLD-SUPP-3001",
                    {"name": "Global Logistics Corp", "rating": 4.9},
                    0.99,
                    ["NAME_EXACT_MATCH"],
                    ["INV-SUPP-12"],
                    "MATCHED",
        ...
        sample_datasets = {
            ...
            "customer_dataset.json": [
                ...
                },
                {
                    ...
                    "company_name": "Nexus Global Dynamics",
                    "arr": 180000,
                    "churn_risk": 0.12,
                    "nps": 8,
                },
            ...
            "inventory_dataset.json": [
                {
                    "sku": "SKU-9001",
                    "product_name": "Ultra-Fast Server Blade",
                ...
                {
                    "sku": "SKU-9002",
                    "product_name": "Fiber Optic Transceiver",
            ...
            "financial_dataset.json": [
                {
                    ...
                    "debit": 4500000.0,
                    "credit": 0.0,
                    "net_balance": 4500000.0,
                },
                {
                    "account_code": "4000",
                    "account_name": "Software Revenue",
                    "period": "2026-Q2",
                    "debit": 0.0,
                    "credit": 12800000.0,
                    "net_balance": 12800000.0,
                },
            ],
            "manufacturing_dataset.json": [
                {
                    "machine_id": "CNC-01",
                    "work_center": "Precision Milling",
                    "units_produced": 8500,
                    "scrap_qty": 42,
                    "oee_percent": 91.4,
                    "downtime_hours": 1.2,
                ...
                {
                    "machine_id": "ROBOT-04",
                    "work_center": "Automated Assembly",
                    "units_produced": 14200,
                    "scrap_qty": 18,
                    "oee_percent": 95.8,
                    "downtime_hours": 0.5,
                },
            ...
            "sales_dataset.json": [
                {
                    "order_id": "ORD-5001",
                    "customer_name": "Apex Enterprise Solutions",
                ...
                {
                    "order_id": "ORD-5002",
                    "customer_name": "Nexus Global Dynamics",
                    "gross_revenue": 32000.0,
                    "discount": 1000.0,
                    "net_margin": 14200.0,
                },
            ],
            "supplier_dataset.json": [
                {
                    "supplier_id": "SUPP-01",
                    "supplier_name": "Global Microelectronics Inc",
                    "otif_rating": 98.4,
                    "lead_time_days": 12,
                ...
                {
                    "supplier_id": "SUPP-02",
                    "supplier_name": "Continental Steel Works",
                    "otif_rating": 94.2,
                    "lead_time_days": 18,
                    "compliance_passed": True,
                },
