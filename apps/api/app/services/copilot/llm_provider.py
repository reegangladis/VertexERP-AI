...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def generate_response(
        ...
        if not self.api_key:
            return await MockLLMProvider(self.model_name).generate_response(messages, tools, temperature)
        ...
        try:
            ...
            logger.error(f"OpenAI completion error: {e}. Falling back to mock response.")
            return await MockLLMProvider(self.model_name).generate_response(messages, tools, temperature)


class AzureOpenAIProvider(BaseLLMProvider):
    def __init__(self, api_key: Optional[str] = None, endpoint: Optional[str] = None, model_name: str = "gpt-4"):
        self.api_key = api_key
...
class LocalModelProvider(BaseLLMProvider):
    def __init__(self, local_url: str = "http://localhost:11434", model_name: str = "llama3"):
    ...
    async def generate_response(
        ...
        try:
            ...
            logger.warning(f"Local LLM client failure ({e}). Falling back to Mock generator.")
            return await MockLLMProvider(self.model_name).generate_response(messages, tools, temperature)
...
class MockLLMProvider(BaseLLMProvider):
    ...
    async def generate_response(
        ...
        if "leave" in user_content or "time off" in user_content:
            ...
            if tools and any(t["function"]["name"] == "check_leave_balance" for t in tools):
                tool_calls = [{
                    "id": f"call_{uuid_str()[:8]}",
                    "type": "function",
                    "function": {
                        "name": "check_leave_balance",
                        "arguments": json.dumps({"employee_id": "current-user-emp"})
                    }
                }]
            else:
                content = (
        ...
        elif "lead" in user_content or "pipeline" in user_content or "crm" in user_content:
            # CRM tool call mock
            if tools and any(t["function"]["name"] == "get_lead_details" for t in tools):
                tool_calls = [{
                    "id": f"call_{uuid_str()[:8]}",
                    "type": "function",
                    "function": {
        ...
        elif "budget" in user_content or "invoice" in user_content or "spend" in user_content:
            # Finance tool call mock
            if tools and any(t["function"]["name"] == "summarize_budget" for t in tools):
                tool_calls = [{
                    "id": f"call_{uuid_str()[:8]}",
                    "type": "function",
                    "function": {
        ...
        elif "inventory" in user_content or "stock" in user_content or "warehouse" in user_content:
        ...
        elif "bom" in user_content or "machine" in user_content or "manufacturing" in user_content:
        ...
        elif "policy" in user_content or "document" in user_content or "manual" in user_content or "search" in user_content:
            # RAG Search mock
            if tools and any(t["function"]["name"] == "search_knowledge_collection" for t in tools):
                tool_calls = [{
                    "id": f"call_{uuid_str()[:8]}",
                    "type": "function",
                    "function": {
        ...
        elif "report" in user_content or "kpi" in user_content or "analytics" in user_content:
            # BI report mock
...
class LLMProviderRegistry:
    ...
    def get_provider(
        ...
        if provider == "openai":
            ...
            return AzureOpenAIProvider(api_key=api_key, endpoint=endpoint, model_name=model_name or "gpt-4")
        elif provider == "gemini":
            return GeminiProvider(api_key=api_key, model_name=model_name or "gemini-1.5-flash")
        elif provider == "anthropic":
            return AnthropicProvider(api_key=api_key, model_name=model_name or "claude-3-5-sonnet")
        elif provider == "local":
            return LocalModelProvider(local_url=local_url or "http://localhost:11434", model_name=model_name or "llama3")
        else:
