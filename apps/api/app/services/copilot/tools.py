...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "employee_id": {
                "type": "string",
                "description": "The target employee ID (UUID format) or 'current'.",
            }
        },
...
async def check_leave_balance(
    db: AsyncSession, org_id: Any, employee_id: str = "current"
) -> dict[str, Any]:
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "limit": {
                "type": "integer",
                "description": "Maximum number of leads to fetch.",
                "default": 5,
            }
        },
    },
...
async def get_lead_details(
    db: AsyncSession, org_id: Any, limit: int = 5
) -> list[dict[str, Any]]:
    try:
    ...
    return [
        {
            ...
            "contact_name": "Alice Miller",
            "deal_value": 45000.0,
            "status": "new",
            "source": "web",
        },
        {
            "company_name": "ByteSize Solutions",
...
async def get_stock_level(
    db: AsyncSession, org_id: Any, product_sku: str
) -> dict[str, Any]:
    try:
        query = select(Product).where(
            Product.sku == product_sku, Product.organization_id == org_id
        )
        res = await db.execute(query)
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "fiscal_year": {
                "type": "string",
                "description": "The year to review, e.g. '2026'.",
            }
        },
...
async def summarize_budget(
    db: AsyncSession, org_id: Any, fiscal_year: str
) -> dict[str, Any]:
    try:
        ...
        if budgets:
            ...
            return {
                ...
                "budget_segments": [
                    {
                        "department": b.name,
                        "allocated": b.total_amount,
                        "spent": b.total_amount * 0.4,
                    }
                    for b in budgets
                ],
            }
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "bom_id": {
                "type": "string",
                "description": "The BOM unique ID or target SKU identifier.",
            }
        },
        "required": ["bom_id"],
    },
...
async def list_bom_items(db: AsyncSession, org_id: Any, bom_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "raw_materials": [
            ...
            {
                "material": "Standard Circuit Assembly",
                "qty_needed": 1.0,
                "unit": "unit",
            },
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "query": {
                "type": "string",
                "description": "Search prompt or query string.",
            },
            "category": {
                "type": "string",
                "description": "Category filter like 'hr', 'finance', 'policies', etc.",
                "default": "general",
            },
        },
        "required": ["query"],
...
async def search_knowledge_collection(
    db: AsyncSession, org_id: Any, query: str, category: str = "general"
) -> list[dict[str, Any]]:
    try:
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "kpi_code": {
                "type": "string",
                "description": "The KPI code name, e.g. 'MRR_KPI' or 'CUSTOMER_CAC'.",
            }
        },
        "required": ["kpi_code"],
    },
...
async def get_kpi_value(db: AsyncSession, org_id: Any, kpi_code: str) -> dict[str, Any]:
    try:
        query = select(KPI).where(KPI.code == kpi_code, KPI.organization_id == org_id)
        res = await db.execute(query)
...
@registry.register(
    ...
    parameters_schema={
        ...
        "properties": {
            "recipient_id": {
                "type": "string",
                "description": "UUID of recipient or 'self'.",
            },
...
async def execute_workflow(
    ...
    for i, step in enumerate(steps):
        ...
        if resp["status"] == "error":
            logs.append(
                f"Failure at step {i+1}: Tool '{t_name}' returned error: {resp['error']}."
            )
            has_failed = True
            break
