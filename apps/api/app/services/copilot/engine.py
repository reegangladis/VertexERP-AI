...
class PIIMasker:
    ...
    SALARY_PATTERN = re.compile(
        r"\b(?:salary|wage|compensation|earn|paid)\s*(?:of|is|equals)?\s*(?:\$?\s*\d+(?:,\d{3})*(?:\.\d{2})?)\b",
        re.IGNORECASE,
    )
    ...
    def render_template(template_str: str, variables: dict[str, Any]) -> str:
        ...
        try:
            ...
            for k, v in variables.items():
                rendered = rendered.replace(f"{{{{ {k} }}}}", str(v)).replace(
                    f"{{{{{k}}}}}", str(v)
                )
            return rendered

    @staticmethod
    def get_default_prompt(department: str | None = None) -> str:
...
class ContextManager:
    @staticmethod
    def compile_history(
        system_prompt: str, chat_history: list[dict[str, Any]], max_messages: int = 15
    ) -> list[dict[str, str]]:
        """
