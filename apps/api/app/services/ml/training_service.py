...
class TrainingService:
    ...
    @staticmethod
    def train_test_split(data: List[Any], test_size: float = 0.2, seed: int = 42) -> Tuple[List[Any], List[Any]]:
        random.seed(seed)
        shuffled = data.copy()
    ...
    @staticmethod
    def k_fold_cross_validation(data: List[Any], k: int = 5) -> List[Tuple[List[Any], List[Any]]]:
        n = len(data)
        fold_size = n // k
        folds = []
        for i in range(k):
    ...
    async def create_training_job(self, organization_id: uuid.UUID, data: MLTrainingJobCreate) -> MLTrainingJob:
        job = MLTrainingJob(
            ...
            hyperparameters_json=data.hyperparameters_json or {"n_estimators": 100, "learning_rate": 0.05, "max_depth": 6},
            status="PENDING",
        )
    ...
    async def execute_training_job(self, job_id: uuid.UUID) -> MLTrainingJob:
        ...
        synthetic_y = [random.choice([0, 1]) for _ in range(200)] if job.model_type == "CLASSIFICATION" else [random.uniform(10, 100) for _ in range(200)]
        ...
        if job.model_id:
            ...
            y_pred = [res["class"] if isinstance(res, dict) else res for res in adapter.predict(synthetic_X)]
