...
class EvaluationService:
    ...
    def compute_regression_metrics(
        ...
        mape_elements = [
            abs((yt - yp) / yt) for yt, yp in zip(y_true, y_pred) if yt != 0
        ]
        mape = (
            (sum(mape_elements) / len(mape_elements) * 100.0) if mape_elements else 0.0
        )
    ...
    async def record_evaluation(
        ...
        recorded_metrics = []
        feature_importance = {
            feat: round(random.uniform(0.05, 0.45), 4)
            for feat in (feature_names or ["f1", "f2", "f3"])
        }

        if model_type.upper() == "CLASSIFICATION":
            metrics = self.compute_classification_metrics(
                [int(x) for x in y_true], [int(x) for x in y_pred]
            )
