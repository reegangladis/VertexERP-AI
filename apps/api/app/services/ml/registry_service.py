...
from app.schemas.ml import MLModelCreate, ModelVersionApprove, ModelVersionCreate
...
class ModelRegistryService:
    ...
    async def register_model(
        self, organization_id: uuid.UUID, data: MLModelCreate
    ) -> MLModel:
        model = MLModel(
    ...
    async def create_model_version(
        self, model_id: uuid.UUID, data: ModelVersionCreate
    ) -> ModelVersion:
        version = ModelVersion(
            ...
            or f"s3://vertex-ml-registry/{model_id}/{data.version}/model.bin",
            approval_status="PENDING",
        )
        return await self.repo.create_model_version(version)

    async def approve_model_version(
        self, version_id: uuid.UUID, approve_data: ModelVersionApprove
    ) -> ModelVersion | None:
    ...
    async def promote_version_stage(
        self, version_id: uuid.UUID, target_stage: str
    ) -> ModelVersion | None:
