...
class CategoricalEncoder:
    ...
    def one_hot_encode(data: List[Dict[str, Any]], column: str) -> List[Dict[str, Any]]:
        categories = sorted(list({str(row[column]) for row in data if column in row and row[column] is not None}))
        encoded_data = []
        for row in data:
            new_row = row.copy()
    ...
    @staticmethod
    def ordinal_encode(data: List[Dict[str, Any]], column: str, mapping: Optional[Dict[str, int]] = None) -> List[Dict[str, Any]]:
        if not mapping:
            categories = sorted(list({str(row[column]) for row in data if column in row and row[column] is not None}))
...
class DataScaler:
    ...
    @staticmethod
    def min_max_scaler(values: List[float], min_val: float = 0.0, max_val: float = 1.0) -> List[float]:
        if not values:
            return []
        val_min = min(values)
        val_max = max(values)
...
class MissingValueHandler:
    ...
    @staticmethod
    def impute_numerical(values: List[Optional[float]], strategy: str = "mean", fill_value: float = 0.0) -> List[float]:
        ...
        return [v if (v is not None and not math.isnan(v)) else impute_val for v in values]
...
class FeatureSelector:
    ...
    @staticmethod
    def filter_low_variance(data: List[Dict[str, float]], threshold: float = 0.01) -> List[str]:
        if not data:
            return []
    ...
    def fit_transform(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for step in self.steps:
            ...
            if action == "one_hot" and column:
                processed_data = CategoricalEncoder.one_hot_encode(processed_data, column)
