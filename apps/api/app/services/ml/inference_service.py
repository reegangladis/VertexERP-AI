...
class InferenceService:
    ...
    async def predict_realtime(
        ...
        if request.model_code:
            model_obj = await self.repo.get_model_by_code(organization_id, request.model_code)
        ...
        num_vals = [float(v) if isinstance(v, (int, float)) else 1.0 for v in feature_vals]
        ...
        prediction_rec = MLPrediction(
            ...
            output_data_json=pred_output if isinstance(pred_output, dict) else {"result": pred_output},
    ...
    async def predict_batch(
        ...
        for input_item in request.batch_input_data:
            ...
            prediction_rec = MLPrediction(
                ...
                output_data_json=pred_res if isinstance(pred_res, dict) else {"result": pred_res},
    ...
    async def get_prediction_history(self, organization_id: uuid.UUID) -> List[MLPrediction]:
