...
def test_input_sanitization_xss():
    ...
    raw_xss = '<script>alert("XSS")</script><img src="x" onerror="javascript:alert(1)"/>'
...
def test_retry_with_jitter_and_fallback():
    ...
    # Fallback test
    fallback_res = engine.execute_with_fallback(
        lambda: 1 / 0,
...
def test_disaster_recovery_restore_simulation():
    ...
    restore = svc.simulate_disaster_recovery_restore(backup["id"], "disaster_recovery_standby")
