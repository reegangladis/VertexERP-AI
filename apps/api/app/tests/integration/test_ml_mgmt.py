...
from app.schemas.ml import MLModelCreate, ModelVersionCreate, MLTrainingJobCreate, MLExperimentCreate, MLPredictionRequest
...
async def test_model_registry_integration_flow():
    ...
    model = await service.register_model(org_id, MLModelCreate(
    ...
    approved = await service.approve_model_version(version_id, MagicMock(approved_by="Lead ML Engineer"))
...
async def test_inference_service_predict_flow():
    ...
    pred = await service.predict_realtime(org_id, MLPredictionRequest(
