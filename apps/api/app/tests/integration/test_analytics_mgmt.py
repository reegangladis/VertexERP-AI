...
@pytest.mark.asyncio
async def test_executive_dashboard_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
    )
...
@pytest.mark.asyncio
async def test_inventory_analytics_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
...
@pytest.mark.asyncio
async def test_finance_analytics_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
...
@pytest.mark.asyncio
async def test_manufacturing_analytics_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
...
@pytest.mark.asyncio
async def test_report_execution_and_export_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
    )
    exec_payload = {"domain": "FINANCE", "page": 1, "page_size": 20}
...
@pytest.mark.asyncio
async def test_analytics_search_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    mock_db_session.execute = AsyncMock(
        side_effect=lambda *args, **kwargs: make_mock_result()
