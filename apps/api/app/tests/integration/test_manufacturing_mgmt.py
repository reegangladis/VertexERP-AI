...
@pytest.mark.asyncio
async def test_get_manufacturing_dashboard(
...
@pytest.mark.asyncio
async def test_list_work_centers_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    current_u = MagicMock()
    current_u.organization_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_list_production_orders_endpoint(
    client: AsyncClient, mock_db_session: MagicMock
):
    current_u = MagicMock()
    current_u.organization_id = uuid.uuid4()
...
async def test_execute_mrp_endpoint(client: AsyncClient, mock_db_session: MagicMock):
    ...
    try:
        ...
        response = await client.post(
            "/api/v1/manufacturing/mrp/runs", json={"run_number": "MRP-INT-001"}
        )
