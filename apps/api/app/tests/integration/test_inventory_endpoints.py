...
@pytest.mark.asyncio
async def test_inventory_summary_telemetry(client: AsyncClient, mock_db_session: MagicMock):
    current_u = MagicMock()
    current_u.organization_id = uuid.uuid4()
    current_u.id = uuid.uuid4()
...
async def test_create_product_validation(client: AsyncClient, mock_db_session: MagicMock):
    ...
    try:
        ...
        response = await client.post("/api/v1/inventory/products", json={"name": "Widget Alpha"})
