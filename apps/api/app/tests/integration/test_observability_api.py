...
from app.models.observability import SystemMetric
...
async def test_observability_health_endpoint(client: AsyncClient, mock_db_session):
    ...
    with (
        patch("app.core.dependencies.oauth2_scheme", return_value="mocked_token"),
        ...
        patch("app.repositories.user.UserRepository.get_with_roles", return_value=mock_user),
        patch(
            "app.core.dependencies.get_current_tenant_id",
            return_value=mock_user.organization_id,
        ),
        patch(
            "app.services.observability_service.ObservabilityService.get_system_health",
...
async def test_record_and_query_metrics(client: AsyncClient, mock_db_session):
    ...
    with (
        ...
        patch(
            "app.services.observability_service.ObservabilityService.record_metric",
            return_value=mock_metric,
        ),
        patch(
            "app.services.observability_service.ObservabilityService.get_system_metrics",
            return_value=[mock_metric],
        ),
    ):
        ...
        response = await client.post(
            "/api/v1/observability/metrics", json=payload, headers=headers
        )
        ...
        response = await client.get(
            "/api/v1/observability/metrics?metric_name=cpu_usage", headers=headers
        )
        assert response.status_code == status.HTTP_200_OK
...
async def test_query_logs(client: AsyncClient, mock_db_session):
    ...
    with (
        ...
        patch(
            "app.services.observability_service.ObservabilityService.search_logs",
            return_value=([], 0),
        ),
    ):
        ...
        headers = {"Authorization": "Bearer mocked_token"}
        response = await client.get(
            "/api/v1/observability/logs?service_name=rest-api&log_level=ERROR",
            headers=headers,
        )
        assert response.status_code == status.HTTP_200_OK
        payload = response.json()
...
async def test_dashboard_config_endpoint(client: AsyncClient, mock_db_session):
    ...
    with (
        ...
        patch(
            "app.services.observability_service.ObservabilityService.get_dashboard_config",
            return_value=None,
        ),
    ):
        ...
        headers = {"Authorization": "Bearer mocked_token"}
        response = await client.get(
            "/api/v1/observability/dashboards/operations", headers=headers
        )
        assert response.status_code == status.HTTP_200_OK
        payload = response.json()
        assert payload["success"] is True
