...
def test_business_ml_modules():
    attr_res = BusinessMLModulesService.predict_attrition({"satisfaction_score": 1.5, "overtime_hours": 25})
    assert attr_res.module_key == "attrition"
    assert attr_res.risk_level == "HIGH"

    fraud_res = BusinessMLModulesService.predict_fraud({"transaction_amount": 15000, "is_foreign_ip": True, "hour_of_day": 2})
