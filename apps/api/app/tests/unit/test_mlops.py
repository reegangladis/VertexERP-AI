...
async def test_create_deployment():
    ...
    res = await service.create_deployment(
        org_id, payload, "principal.architect@vertex.ai"
    )
...
async def test_update_traffic():
    ...
    dep_id = uuid.uuid4()
    mock_dep = MLDeployment(
        id=dep_id, model_version_id=uuid.uuid4(), target_traffic_percentage=100.0
    )

    service.repo.get_deployment_by_id = AsyncMock(return_value=mock_dep)
    service.repo.update_deployment = AsyncMock(
        return_value=MLDeployment(id=dep_id, target_traffic_percentage=25.0)
    )
    service.repo.create_history = AsyncMock()
...
async def test_rollback_deployment():
    ...
    mock_dep = MLDeployment(id=dep_id, model_version_id=old_version_id, status="ACTIVE")

    service.repo.get_deployment_by_id = AsyncMock(return_value=mock_dep)
    service.repo.update_deployment = AsyncMock(
        return_value=MLDeployment(
            id=dep_id, model_version_id=stable_version_id, status="ACTIVE"
        )
    )
    service.repo.create_history = AsyncMock()

    rolled_back = await service.rollback_deployment(
        dep_id, stable_version_id, "admin@vertex.ai"
    )
...
async def test_run_pipeline():
    ...
    payload = PipelineRunCreate(
        template_id=template_id, run_name="Continuous Training Run #1"
    )
...
async def test_evaluate_drift_triggers_alert():
    ...
    service.repo.get_deployment_by_id = AsyncMock(
        return_value=MLDeployment(id=dep_id, model_id=uuid.uuid4())
    )
    service.trigger_retraining_job = AsyncMock()

    # Low drift - should be normal
    payload_low = DriftReportCreate(
        drift_type="DATA_DRIFT", feature_name="satisfaction_score", drift_score=0.08
    )
    ...
    payload_high = DriftReportCreate(
        drift_type="DATA_DRIFT", feature_name="satisfaction_score", drift_score=0.35
    )
