...
async def test_bom_cost_rollup_calculation():
    ...
    item1 = MagicMock(quantity=2.0, unit_cost=10.0, scrap_factor_percent=5.0, extended_cost=21.0, component_product_id=uuid.uuid4())
    item2 = MagicMock(quantity=1.0, unit_cost=50.0, scrap_factor_percent=0.0, extended_cost=50.0, component_product_id=uuid.uuid4())
...
async def test_mrp_engine_suggestion_generation():
    ...
    mock_exec_orders.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[])))
    
    mock_prod = MagicMock(id=uuid.uuid4(), name="Raw Steel Plate", sku="RAW-STEEL-01", safety_stock=50, reorder_level=20)
    mock_exec_products = MagicMock()
    mock_exec_products.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[mock_prod])))

    mock_wc = MagicMock(id=uuid.uuid4(), name="CNC Center", capacity_per_day_hours=16.0)
    mock_exec_wcs = MagicMock()
    mock_exec_wcs.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[mock_wc])))

    db_mock.execute = AsyncMock(side_effect=[mock_exec_orders, mock_exec_products, mock_exec_wcs])
