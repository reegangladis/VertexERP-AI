...
async def test_business_ml_modules_predictions():
    # HR Attrition
    attr_res = BusinessMLModulesService.predict_attrition({"tenure_months": 12, "satisfaction_score": 2.1, "overtime_hours": 25})
    assert attr_res.module_key == "attrition"
    assert "attrition_probability" in attr_res.prediction_result

    # Sales Forecast
    sales_res = BusinessMLModulesService.predict_sales_forecast({"historical_sales": 150000, "growth_rate": 0.12, "deals_in_pipeline": 15})
    assert sales_res.module_key == "sales_forecasting"
    assert "projected_quarterly_sales" in sales_res.prediction_result

    # Customer Churn
    churn_res = BusinessMLModulesService.predict_customer_churn({"support_tickets_opened": 10, "nps_score": 3, "days_since_last_login": 35})
    assert churn_res.module_key == "churn"
    assert "churn_probability" in churn_res.prediction_result or "churn_risk" in churn_res.prediction_result

    # Inventory Optimization
    inv_res = BusinessMLModulesService.predict_inventory_optimization({"current_stock": 100, "reorder_point": 250, "holding_cost": 4.5})
    assert inv_res.module_key == "inventory_opt"
    assert "reorder_flag" in inv_res.prediction_result

    # Fraud Detection
    fraud_res = BusinessMLModulesService.predict_fraud({"transaction_amount": 25000, "is_foreign_ip": True, "hour_of_day": 3})
