...
async def test_balanced_journal_entry():
    ...
    service.journal_repo.create = AsyncMock(
        return_value=MagicMock(id=uuid.uuid4(), entry_number="JE-TEST-001")
    )
    service.journal_repo.get_with_lines = AsyncMock(
        return_value=MagicMock(id=uuid.uuid4(), entry_number="JE-TEST-001", lines=[])
    )
...
async def test_account_creation_and_seeding():
    ...
    service.account_repo.get_by_code = AsyncMock(
        return_value=MagicMock(id=uuid.uuid4(), account_code="1010")
    )
    account_in = AccountCreate(
        account_code="1010", account_name="Cash", account_type="Assets"
    )
...
async def test_journal_entry_reversal():
    ...
    service.create_journal_entry = AsyncMock(
        return_value=MagicMock(id=uuid.uuid4(), entry_number="REV-JE-001")
    )
    service.post_journal_entry = AsyncMock(
        return_value=MagicMock(
            id=uuid.uuid4(), entry_number="REV-JE-001", status="POSTED"
        )
    )
...
async def test_credit_note_creation():
    ...
    cn_data = CreditNoteCreate(
        customer_id=uuid.uuid4(), amount=75.0, reason="Sales Discount"
    )

    service.credit_note_repo.create = AsyncMock(
        return_value=MagicMock(
            id=uuid.uuid4(),
            organization_id=org_id,
...
async def test_payment_and_bank_balance_update():
    ...
    service.payment_repo.create = AsyncMock(
        return_value=MagicMock(id=uuid.uuid4(), payment_number="PAY-001", amount=150.0)
    )
...
async def test_reports_and_dashboard_summary():
    ...
    service.get_profit_loss = AsyncMock(
        return_value=MagicMock(
            total_revenue=1000.0, total_expenses=400.0, net_profit=600.0
        )
    )
    service.get_invoices = AsyncMock(
        return_value=[MagicMock(total_amount=500.0, paid_amount=100.0)]
    )
    service.get_bills = AsyncMock(
        return_value=[MagicMock(total_amount=300.0, paid_amount=100.0)]
    )
    service.get_bank_accounts = AsyncMock(
        return_value=[MagicMock(current_balance=2500.0)]
    )
