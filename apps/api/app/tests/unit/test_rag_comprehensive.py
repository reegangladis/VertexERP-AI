...
async def test_faiss_vector_db_and_hybrid_retrieval():
    ...
    text_sample = (
        "Financial policy requires all expense reports to be submitted within 30 days."
    )
...
async def test_rag_pipeline_answer_and_citations():
    ...
    text_sample = (
        "Safety compliance rule: Eye protection is mandatory in manufacturing zone B."
    )
    vec = await embedding_service.get_embedding(text_sample, provider="openai")
