...
export const dataEngineeringApi = {
  getMonitoringSummary: async (): Promise<DataEngineeringMonitoringSummary> => {
    try {
      const response = await api.get('/data-engineering/monitoring/summary');
  ...
  getETLJobs: async (): Promise<ETLJob[]> => {
    try {
      const response = await api.get('/data-engineering/etl-jobs');
      return response.data;
    } catch {
      return [
        {
  ...
  triggerPipelineRun: async (jobId: string): Promise<ETLRun> => {
    try {
      const response = await api.post(`/data-engineering/etl-jobs/${jobId}/run`);
      return response.data;
    } catch {
      return {
        id: 'run-999',
  ...
  getDatasets: async (): Promise<Dataset[]> => {
    try {
      const response = await api.get('/data-engineering/datasets');
      return response.data;
    } catch {
      return [
        {
  ...
  getFeatureGroups: async (): Promise<FeatureGroup[]> => {
    try {
      const response = await api.get('/data-engineering/feature-groups');
      return response.data;
    } catch {
      return [
        {
  ...
  getDataQualityReports: async (): Promise<DataQualityReport[]> => {
    try {
      const response = await api.get('/data-engineering/data-quality');
      return response.data;
    } catch {
      return [
        {
  ...
  getLineageGraph: async (): Promise<LineageGraph> => {
    try {
      const response = await api.get('/data-engineering/lineage');
      return response.data;
    } catch {
      return {
        nodes: [
  ...
  getMetadataCatalog: async (): Promise<any[]> => {
    try {
      const response = await api.get('/data-engineering/metadata');
      return response.data;
    } catch {
      return [
        {
