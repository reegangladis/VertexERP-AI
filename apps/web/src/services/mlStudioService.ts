...
export const mlStudioService = {
  ...
  async getDatasets(domain?: string): Promise<DatasetItem[]> {
    const res = await api.get('/api/v1/ml-studio/datasets', { params: { domain } });
    return res.data;
  },
  async createDataset(data: any): Promise<DatasetItem> {
    const res = await api.post('/api/v1/ml-studio/datasets', data);
    return res.data;
  },
  async getDatasetPreview(id: string): Promise<any> {
    const res = await api.get(`/api/v1/ml-studio/datasets/${id}/preview`);
    return res.data;
  },
  async validateDataset(id: string): Promise<any> {
    const res = await api.post(`/api/v1/ml-studio/datasets/${id}/validate`);
    return res.data;
  },

  // Notebooks
  async getNotebooks(): Promise<NotebookItem[]> {
    const res = await api.get('/api/v1/ml-studio/notebooks');
    return res.data;
  },
  async createNotebook(data: any): Promise<NotebookItem> {
    const res = await api.post('/api/v1/ml-studio/notebooks', data);
    return res.data;
  },
  async executeNotebook(id: string): Promise<any> {
    const res = await api.post(`/api/v1/ml-studio/notebooks/${id}/execute`);
    return res.data;
  },
  async getNotebookTemplates(): Promise<any[]> {
    const res = await api.get('/api/v1/ml-studio/notebooks/templates');
    return res.data;
  },

  // Models & Approval Workflow
  async getModels(stage?: string): Promise<RegisteredModelItem[]> {
    const res = await api.get('/api/v1/ml-studio/models', { params: { stage } });
    return res.data;
  },
  async registerModel(data: any): Promise<RegisteredModelItem> {
    const res = await api.post('/api/v1/ml-studio/models', data);
    return res.data;
  },
  async approveModel(id: string, approval_status: string, approved_by: string, approval_notes?: string): Promise<RegisteredModelItem> {
    const res = await api.post(`/api/v1/ml-studio/models/${id}/approve`, {
      approval_status,
      approved_by,
      approval_notes,
    });
  ...
  async promoteModel(id: string, stage: string): Promise<RegisteredModelItem> {
    const res = await api.post(`/api/v1/ml-studio/models/${id}/promote`, { stage });
    return res.data;
  },

  // Evaluation & Explainability
  async getEvaluationReport(modelId: string): Promise<EvaluationReportItem[]> {
    const res = await api.get(`/api/v1/ml-studio/evaluations/${modelId}`);
    return res.data;
  },
  async getExplainabilityReport(modelId: string): Promise<ExplainabilityReportItem[]> {
    const res = await api.get(`/api/v1/ml-studio/explainability/${modelId}`);
    return res.data;
  },
  async explainLocalPrediction(data: any): Promise<any> {
    const res = await api.post('/api/v1/ml-studio/explainability/local-explain', data);
    return res.data;
  },

  // Comparison & Packaging
  async compareModels(): Promise<ModelComparisonResponse> {
    const res = await api.get('/api/v1/ml-studio/models/compare');
    return res.data;
  },
  async preparePackaging(modelId: string): Promise<any> {
    const res = await api.post(`/api/v1/ml-studio/packaging/${modelId}/prepare`);
    return res.data;
  },
};
